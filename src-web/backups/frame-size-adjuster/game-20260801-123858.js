(() => {
  'use strict';
  const W=1280,H=720,WORLD_W=6200,WORLD_BOTTOM=920,GROUND_Y=570,UNDER_Y=1280,GROUND_MIN_Y=490,GROUND_MAX_Y=760, canvas=document.querySelector('#game'), g=canvas.getContext('2d');
  const {
    ...
    spinnerSpinSheet,barbarianReviveSheet,barbarianSwingFxSheet,
  ...
  let running=false,last=0,shake=0,flash=0,slow=0,wave=1,message='',messageT=0,screenTint=0,hitFx=[],blastFx=[],resourceFx=[],cameraX=0,cameraY=0,mapIndex=0,gateIndex=0,mapCycle=0,gatePhase='combat',stageLockX=70,stageRightX=1050,cameraLockX=0,powerSwitch=null,gauntletMode=false,gauntletIndex=0,freeTourMode=false,failureHandled=false,failurePopupT=0,lostResources=null,pendingReward=null,rewardResumeRunning=false,jumpSourcePlatform=null,jumpPlatformCleared=false;
  ...
  function pushOutOfGroundWalls(a,pad=0){
    if(a.level!==0)return false;
    if(isGroundAccessLane(a,pad))return false;
    const ground=groundWalkBounds();
    let blocked=false;
    for(const p of currentPlatforms()){
      const wall=platformWallRect(p),top=Math.max(ground.min,wall.top),bottom=Math.min(ground.max,wall.bottom);
      if(bottom<=top||a.x<wall.left-pad||a.x>wall.right+pad||a.y<top-pad||a.y>bottom+pad)continue;
      // A ground-level actor may pass around a wall, or stay in front of it,
      // but may never stand on the visible wall face or behind its covered road.
      const toLeft=Math.abs(a.x-(wall.left-pad)),toRight=Math.abs(a.x-(wall.right+pad)),toFront=Math.abs(a.y-(bottom+pad));
      if(toFront<=toLeft&&toFront<=toRight)a.y=bottom+pad;
      else if(toLeft<toRight)a.x=wall.left-pad;
      else a.x=wall.right+pad;
      blocked=true
    }
    return blocked
  }
  ...
  function updateCamera(){
    ...
    cameraY=Math.min(player.y-H*.58,maxY)
  }
  ...
  const player={x:270,y:566,z:0,vz:0,knockVx:0,level:0,face:1,hp:160,maxHp:160,state:'idle',timer:0,combo:0,comboT:0,kickCombo:0,kickComboT:0,grab:null,grabTarget:null,grabT:0,grabEscapeT:0,grabKickQueue:0,grabKickTotal:0,risingQueued:false,risingCooldown:0,risingAirInv:false,risingInvT:0,held:null,heldUses:0,inv:0,kills:0,inputQueue:[],climb:null,fallDamage:0,fallFromLevel:null,fallStartY:null,subPit:null,pitSafeT:0};
  let enemies=[],companions=[],pickups=[],projectiles=[];
  const defaultProgress={gold:0,chicken:0,fruit:0,hpLv:0,atkLv:0,defLv:0,spdLv:0,ascend:0};
  let progress=loadProgress();
  function loadProgress(){try{return{...defaultProgress,...JSON.parse(localStorage.getItem('tiejie-progress')||'{}')}}catch(e){return{...defaultProgress}}}
  function saveProgress(){try{localStorage.setItem('tiejie-progress',JSON.stringify(progress))}catch(e){}}
  function playerMaxHp(){return 160+progress.hpLv*18+progress.ascend*28}
  function playerAtkMul(){return 1+progress.atkLv*.075+progress.ascend*.16}
  function playerDefMul(){return Math.max(.45,1-progress.defLv*.045-progress.ascend*.035)}
  function playerSpeedMul(){return 1+Math.min(5,progress.spdLv)*.06}
  ...
  function tryUpgrade(kind){
    const map={hp:['hpLv','血量'],atk:['atkLv','攻击'],def:['defLv','防御'],spd:['spdLv','速度']},m=map[kind];if(!m)return;
    const cost=2+Math.floor(progress[m[0]]*.55);if(kind!=='spd'||progress.spdLv<5){if(progress.chicken<cost){openRewardModal('chicken',`提升${m[1]}需要 ${cost} 个鸡腿，当前只有 ${progress.chicken} 个。`);return}}
    if(kind==='spd'&&progress.spdLv>=5){if(progress.fruit<1){openRewardModal('fruit','速度已经达到 5 级，进阶需要 1 颗果实。');return}progress.spdLv=0;progress.fruit--;progress.ascend++;progress.atkLv+=2;player.maxHp=playerMaxHp();player.hp=player.maxHp;message='身体像烧起来了……我突破了！';messageT=1.8;saveProgress();return}
    progress.chicken-=cost;progress[m[0]]++;player.maxHp=playerMaxHp();if(kind==='hp')player.hp=player.maxHp;message=`${m[1]}稳了一点……还能更强`;messageT=.9;saveProgress()
  }
  function closeRewardModal(resume=true){const modal=document.querySelector('#reward-modal');if(modal)modal.hidden=true;pendingReward=null;if(resume)running=rewardResumeRunning;rewardResumeRunning=false}
  function openRewardModal(kind,reason='',mode='resource'){
    ...
    const modal=document.querySelector('#reward-modal'),title=document.querySelector('#reward-modal-title'),desc=document.querySelector('#reward-modal-desc'),confirm=document.querySelector('#reward-confirm');if(!modal)return;
    rewardResumeRunning=running;running=false;pendingReward={kind,mode};title.textContent=mode==='revive'?'是否继续这次战斗？':'资源补给';
    desc.textContent=mode==='revive'?`本次未通关，携带的金币、鸡腿和果实已掉落。完整观看视频可找回全部道具并原地复活。`:`${reason||`${names[kind]}不足。`} 完整观看视频可领取 ${names[kind]} +${rewards[kind]}。`;
    confirm.textContent=mode==='revive'?'观看视频，找回并复活':'观看视频并领取';modal.hidden=false
  }
  async function claimRewardedResource(kind,mode='resource'){
    ...
    const buttons=[...document.querySelectorAll('[data-reward-ad],#reward-confirm,#reward-cancel')];buttons.forEach(button=>button.disabled=true);message='正在准备视频……';messageT=99;
    try{
      const placement=mode==='revive'?'revive':kind,result=await window.TieJiePlatform?.ads?.showRewarded(placement);
      if(result?.ok&&mode==='revive'){
        if(lostResources){progress.gold+=lostResources.gold;progress.chicken+=lostResources.chicken;progress.fruit+=lostResources.fruit}lostResources=null;failureHandled=false;saveProgress();
        player.hp=Math.max(1,Math.round(player.maxHp*.5));player.state='idle';player.timer=0;player.z=0;player.vz=0;player.inv=2;message='道具已经找回，重新站起来！';messageT=1.8
      }else if(result?.ok){progress[kind]+=amount;saveProgress();message=`领取完成：${names[kind]} +${amount}`;messageT=1.6}
    ...
    finally{buttons.forEach(button=>button.disabled=false);closeRewardModal(true)}
  }
  function handleRunFailure(){
    ...
    const growthPanel=document.querySelector('#growth-panel');if(growthPanel)growthPanel.hidden=true;openRewardModal('gold','', 'revive')
  }
  function queueRunFailure(){if(failureHandled||failurePopupT>0)return;failurePopupT=3;const growthPanel=document.querySelector('#growth-panel');if(growthPanel)growthPanel.hidden=true}
  ...
  const recruitCatalog=[
    {type:'assassin',price:50},{type:'axe',price:80},{type:'suit',price:120},
    {type:'skinny',price:160},{type:'heavy',price:220},{type:'spinner',price:300},
    {type:'grappler',price:400},{type:'whip',price:520},{type:'barbarian',price:700}
  ];
  ...
  function enemy(x,y,type='skinny',name,elite=false,rank=1){const meta=enemyCatalog[type]||enemyCatalog.skinny,hp=Math.round(meta.hp*(1+rank*.075)*(elite?1.65:1));return{x,y,z:0,vz:0,level:levelAtPoint(x,y),face:-1,faceHold:0,hp,maxHp:hp,type,rank,damage:Math.round(meta.dmg*(1+rank*.045)*(elite?1.25:1)),speed:meta.spd*(1+Math.min(.45,rank*.006)),state:'idle',timer:0,inv:0,attackT:1+Math.random()*1.3,slamT:type==='heavy'?.9+Math.random()*1.4:99,slideT:type==='skinny'?1.1+Math.random()*1.2:99,specialT:type==='barbarian'?2.8+Math.random()*1.8:.8+Math.random()*1.4,slamTargetX:x,slamHit:false,slideHit:false,name:name||meta.name,elite,dead:false,grabbed:false,revived:false,rage:false}}
  ...
  function spawnSelectedCompanions(){
    companions=[];let i=0;
    for(const type of selectedRecruitTypes){
      const c=enemy(player.x-72-(i%3)*58,player.y+42+Math.floor(i/3)*34,type,`队友·${enemyCatalog[type].name}`,false,1);
      c.companion=true;c.team='player';c.face=1;c.level=player.level;c.attackT=.22+i*.08;c.followSlot=i;c.state='idle';syncCompanionStats(c);c.hp=c.maxHp;companions.push(c);i++
    }
  }
  function prepareRecruitment(){
    if(selectedRecruitTypes.size===0)return true;
    const free=!!document.querySelector('#test-free-recruit')?.checked;
    const total=[...selectedRecruitTypes].reduce((sum,type)=>sum+(recruitCatalog.find(r=>r.type===type)?.price||0),0);
    if(!free&&progress.gold<total){refreshRecruitPanel(`金币不足：已选需要 ${total}，当前只有 ${progress.gold}`);openRewardModal('gold',`招募所选队友需要 ${total} 金币，当前只有 ${progress.gold}。`);return false}
    if(!free){progress.gold-=total;saveProgress()}
    return true
  }
  function hurtCompanion(c,dmg,kx,options={}){
    if(c.dead||c.inv>0)return;const real=Math.max(1,Math.round(dmg*(c.defMul||1)));c.hp-=real;c.inv=.32;c.x+=kx*.35;impact(c.x,c.y-c.z-76,real,false,kx);
    if(c.hp<=0&&c.type==='barbarian'&&!c.revived){c.hp=0;c.state='barbarianDown';c.timer=3;c.inv=3;c.z=0;c.vz=0;c.knockVx=0;return}
    if(c.hp<=0){c.hp=0;c.dead=true;c.state='down';c.timer=2.2;message=`${c.name}倒下了，下张地图会归队`;messageT=1.2}else if(options.knockdown){c.state='down';c.timer=.9;c.z=Math.max(c.z||0,18);c.vz=Math.max(c.vz||0,options.launchVz||340)}else{c.state='hurt';c.timer=.3}
  }
  ...
  function companionCanHit(c,t,xRange,yRange=62,zRange=150){return !!(t&&!t.dead&&!t.grabbed&&t.level===c.level&&Math.abs(t.x-c.x)<xRange&&Math.abs(t.y-c.y)<yRange&&zReach(c,t,zRange))}
  ...
  function startCompanionSpecial(c,t,d){
    c.target=t;c.specialT=1.35+Math.random()*.7;c.specialHit=false;c.specialDone=false;
    if(c.type==='barbarian'&&d<(c.rage?380:300)){c.state='barbarianCharge';c.timer=c.rage?.52:.82;c.specialT=c.rage?1.05:3.4+Math.random()*1.2;return true}
    if(c.type==='spinner'){c.state='spinAir';c.timer=.78;c.z=65;c.spinVx=c.face*310;return true}
    if(c.type==='grappler'&&d<145&&Math.abs(t.y-c.y)<64){c.state='enemyGrabWindup';c.timer=.38;return true}
  ...
  function updateCompanionSpecialState(c,dt){
    ...
    if(c.state==='barbarianRevive'){c.timer-=dt;c.inv=Math.max(c.inv,.12);if(c.timer<=0){c.revived=true;c.rage=true;c.hp=Math.max(1,Math.round(c.maxHp*.38));c.state='barbarianUppercut';c.timer=.56;c.specialHit=false;c.inv=.65;c.specialT=1.2}return true}
    ...
    if(c.state==='enemyGrabWindup'){c.timer-=dt;if(t&&!t.dead)c.y+=(t.y-c.y)*Math.min(1,dt*8);if(c.timer<=0){if(companionCanHit(c,t,110,55,110)){t.grabbed=true;t.state='grabbed';c.state='enemyGrabbed';c.timer=.42}else c.state='idle'}return true}
    if(c.state==='enemyGrabbed'){c.timer-=dt;if(!t||t.dead){if(t)t.grabbed=false;c.state='idle';return true}t.x=c.x+c.face*40;t.y=c.y;if(c.timer<=0){t.grabbed=false;c.state='enemyThrow';c.timer=.6;c.specialHit=false}return true}
    if(c.state==='enemyThrow'){c.timer-=dt;if(!c.specialHit&&companionStrike(c,t,1.18,690,{range:94,yRange:60,zReach:180,knockdown:true,launchVz:740})){c.specialHit=true}if(c.timer<=0)c.state='idle';return true}
  ...
  function updateCompanion(c,dt){
    ...
    for(const e of enemies){if(e.dead||e.grabbed||e.level!==c.level)continue;const d=dist(c,e);if(d<best){best=d;target=e}}
    if(!target){
    ...
    if(c.type==='heavy'&&c.slamT<=0&&d<225){c.state='slamCharge';c.timer=.72;c.slamTargetX=target.x;c.attackT=1.2;c.slamT=3.35;return}
    if(c.type==='skinny'&&c.slideT<=0&&d<310){c.state='slideWindup';c.timer=.24;c.slideT=1.9+Math.random()*.7;return}
    if(c.type==='whip'){
  ...
  function spawnGate(){
    const map=trialMaps[mapIndex],gate={start:120,end:6040,boss:true,enemies:map.gates.flatMap(g=>g.enemies)},rank=mapCycle*trialMaps.length*5+mapIndex*5+1;currentStageTheme=map.theme;gatePhase='combat';stageLockX=35;stageRightX=WORLD_W-35;cameraLockX=0;enemies=[];
    ...
    const spots=enemyStandSpots(gate,gate.enemies.length),platformSpots=spots.filter(s=>s.kind==='platform').sort((a,b)=>a.x-b.x),used=[];
    const specs=gate.enemies.slice();
    while(specs.length<platformSpots.length){const index=specs.length;specs.push({type:index%2?'heavy':'skinny',dx:220+index*260,y:570})}
    for(let i=0;i<specs.length;i++){
      ...
      let spot=platformSpots[i]||spots.find(s=>!used.includes(s)&&s.kind==='ground')||spots.find(s=>!used.includes(s)&&s.kind==='pit')||spots.find(s=>!used.includes(s))||{x:clamp(gate.start+(spec.dx||220+i*260),gate.start+180,gate.end-160),y:spec.y||570,level:0};
      used.push(spot);
      const e=enemy(spot.x,spot.y,type,name,elite,rank);e.level=spot.level;e.subPit=spot.subPit||null;e.homeX=spot.x;e.homeY=spot.y;e.awake=false;enemies.push(e)
    }
  ...
  function loadMap(index){
    gauntletMode=false;failureHandled=false;failurePopupT=0;lostResources=null;jumpSourcePlatform=null;jumpPlatformCleared=false;closeRewardModal(false);
    const maxHp=playerMaxHp();mapIndex=index;gateIndex=0;gatePhase='combat';stageLockX=70;stageRightX=1050;cameraLockX=0;cameraX=0;currentStageTheme=trialMaps[mapIndex].theme;generatedTerrain=generateFreshTerrain(currentStageTheme,mapIndex);Object.assign(player,{x:220,y:566,z:0,vz:0,knockVx:0,level:0,face:1,hp:maxHp,maxHp,state:'idle',timer:0,combo:0,comboT:0,kickCombo:0,kickComboT:0,grab:null,grabTarget:null,grabT:0,grabEscapeT:0,grabKickQueue:0,grabKickTotal:0,risingQueued:false,risingCooldown:0,risingAirInv:false,risingInvT:0,held:null,heldUses:0,inv:0,kills:player.kills||0,inputQueue:[],climb:null,fallDamage:0,fallFromLevel:null,fallStartY:null,subPit:null,pitSafeT:0});
    spawnSelectedCompanions();pickups=freeTourMode?[]:[{type:'grenade',x:620,y:570,level:0,active:true},{type:'hammer',x:860,y:570,level:0,active:true,throwsLeft:2},{type:'grenade',x:2350,y:575,level:0,active:true},{type:'hammer',x:3920,y:570,level:0,active:true,throwsLeft:2},{type:'grenade',x:5200,y:570,level:0,active:true}];projectiles=[];hitFx=[];blastFx=[];resourceFx=[];screenTint=0;wave=1;spawnGate();updateCamera()
  }
  function reset(){player.kills=0;mapCycle=0;loadMap(0)}
  function spawnGauntletEnemy(){
    ...
    const e=enemy(spot.x,spot.y,type,enemyName(type,rank+17),false,rank);e.level=spot.level;e.subPit=spot.subPit||null;e.homeX=e.x;e.homeY=e.y;e.awake=false;enemies=[e];
    powerSwitch=null;currentStageTheme='沙漠';
    player.hp=Math.min(player.maxHp,player.hp+34);
    message=`轮番战 ${gauntletIndex+1} / ${gauntletEnemyTypes.length}：${enemyCatalog[type].name}`;
    messageT=1.6;
  }
  function startGauntlet(){
    gauntletMode=true;failureHandled=false;failurePopupT=0;lostResources=null;jumpSourcePlatform=null;jumpPlatformCleared=false;closeRewardModal(false);gauntletIndex=0;wave=1;mapCycle=0;mapIndex=1;gateIndex=0;gatePhase='combat';stageLockX=70;stageRightX=1860;cameraLockX=0;currentStageTheme='沙漠';generatedTerrain=generateThemeTerrain(currentStageTheme,0x7eed1234);generatedTerrain.pits=[normalizePit({x:1460,w:118,name:'测试暗坑'},'沙漠',Math.random)];desertPits=generatedTerrain.pits;
    ...
    spawnSelectedCompanions();pickups=[{type:'hammer',x:520,y:570,level:0,active:true,throwsLeft:2},{type:'grenade',x:650,y:570,level:0,active:true}];projectiles=[];hitFx=[];blastFx=[];resourceFx=[];screenTint=0;spawnGauntletEnemy();updateCamera()
  }
  function act(name,fromQueue=false){if(!running||player.hp<=0||player.climb)return;if(player.state==='hurt'||player.state==='down'||player.state==='fall')return;
    ...
    if(name==='jump'){if(player.z===0&&!player.grab){jumpSourcePlatform=currentPlatforms().find(p=>insidePlatform(player.x,player.y,10,p))||null;jumpPlatformCleared=false;player.vz=0;player.risingQueued=false;player.risingAirInv=false;player.risingInvT=0;player.state='jumpCrouch';player.timer=.14}return}
    if(name==='grab'){
    ...
    if(name==='kick'){
      if(player.state==='jump'||player.z>0){player.state='airBackKick';player.timer=.62;combatAttack(136,31,640,285,24,{knockdown:true,launchVz:420,zReach:128});message='空中也别想近我身！';messageT=.62;return}
  ...
  function canGrabEnemy(e){return !!(enemyCatalog[e.type]?.grab)}
  function zReach(a,b,reach=96){const az=a.z||0,bz=b.z||0;return Math.abs(az-bz)<=reach||(az===0&&bz<=reach)||(bz===0&&az<=reach)}
  function finishGrabAttempt(){const e=player.grabTarget,ungrab=nearestUngrabbable(96);player.grabTarget=null;if(!e||!canGrabEnemy(e)||e.dead||e.grabbed||e.level!==player.level||dist(player,e)>96||!zReach(player,e,108)){player.state='grabRelease';player.timer=.28;message=(e&&!canGrabEnemy(e))||ungrab?`这家伙太沉了……我抓不住`:'手空了……得挨打了';messageT=.65;return}player.grab=e;player.grabT=0;player.grabEscapeT=GRAB_MAX_DURATION;player.grabKickQueue=0;player.grabKickTotal=0;e.grabbed=true;e.state='grabbed';e.z=player.z||0;e.vz=player.vz||0;player.state='grab';player.timer=.15;hitStop(.06);message='抓住后不能移动……五秒内出手';messageT=2.4}
  ...
  function hurtEnemy(e,dmg,kx,options={}){
    const wasAirLaunch=e.airLaunch&&e.state==='knockdown',airZ=e.z||0,airVz=e.vz||0,airCarryX=e.throwVx||e.knockVx||0,wasAirborne=airZ>0||Math.abs(airVz)>1||e.airLaunch||e.state==='thrown'||e.state==='slamAir';e.hp-=dmg;e.inv=wasAirLaunch?.08:.22;impact(e.x,e.y-e.z-86,dmg,false,kx);hitStop(dmg>20?.1:.055);
  ...
  function startKneeStrike(){if(!player.grab)return;const strikeNo=Math.max(1,player.grabKickTotal-player.grabKickQueue);player.state='grabKnee';player.timer=.56;setTimeout(()=>strikeGrab(11,`膝撞 ${strikeNo}`),190)}
  function strikeGrab(dmg,label){const e=player.grab;if(!e)return;e.hp-=dmg;impact(e.x,e.y-96,dmg,false,player.face*120);hitStop(.09);message=label+'！';messageT=.5;if(e.hp<=0){e.grabbed=false;e.dead=true;e.state=e.type==='heavy'?'heavyDefeated':'down';e.timer=1.05;e.z=0;e.vz=0;player.kills++;clearGrab();player.state='grabRelease';player.timer=.28}}
  ...
  function hurtPlayer(dmg,kx,options={}){const chainable=player.z>0||player.vz>0||player.state==='hurt'||player.state==='enemyGrabbed';if(player.risingAirInv||player.hp<=0||(player.inv>0&&!options.ignoreInv&&!chainable))return;if(player.grab)releaseGrab(false);player.grabTarget=null;player.risingQueued=false;player.risingAirInv=false;player.risingInvT=0;const held=player.state==='enemyGrabbed',real=Math.max(1,Math.round(dmg*playerDefMul()));player.hp-=real;player.inv=options.inv??(chainable?.08:.22);player.state=player.hp<=0?'down':held?'enemyGrabbed':'hurt';player.timer=player.hp<=0?2:(options.timer??.44);player.inputQueue.length=0;if(!held){player.x+=kx*1.25;player.knockVx=(player.knockVx||0)+kx*.28;if(player.hp>0&&options.lift!==false){player.z=Math.max(player.z||0,chainable?18:10);player.vz=Math.max(player.vz||0,options.liftVz||(chainable?260:190))}}impact(player.x,player.y-player.z-88,real,true,kx*9);hitStop(options.hitStop??.075);if(player.hp<=0){message='撑不住了……';messageT=3;queueRunFailure()}}
  function knockPlayerDown(dmg,kx,options={}){const chainable=player.z>0||player.vz>0||player.state==='hurt'||player.state==='enemyGrabbed';if(player.risingAirInv||player.hp<=0||(player.inv>0&&!options.ignoreInv&&!chainable))return;if(player.grab)releaseGrab(false);player.grabTarget=null;player.risingQueued=false;player.risingAirInv=false;player.risingInvT=0;const held=player.state==='enemyGrabbed'&&!options.breakHold;player.hp-=dmg;player.inv=options.inv??(chainable?.08:.3);player.state=player.hp<=0?'down':held?'enemyGrabbed':'down';player.timer=player.hp<=0?2.2:.9;player.inputQueue.length=0;if(!held){player.z=Math.max(player.z||0,18);player.vz=Math.max(player.vz||0,options.launchVz||(chainable?420:340));player.knockVx=(player.knockVx||0)+kx*.75;player.x+=kx*.65}impact(player.x,player.y-player.z-52,dmg,true,kx*9);hitStop(.1);message=player.hp<=0?'撑不住了……':'腿被扫了，起身要快！';messageT=player.hp<=0?3:.8;if(player.hp<=0)queueRunFailure()}
  ...
  function enemyUseStairs(e,dt){
    if(e.level===player.level)return false;
    const up=player.level>e.level,s=currentStairs().find(st=>up?st.fromLevel===e.level:st.toLevel===e.level);if(!s)return true;
    ...
    const step=(e.type==='skinny'?88:e.elite?62:54)*dt;e.y+=up?-step:step;e.x+=(stairCenterAtY(s,e.y)-e.x)*Math.min(1,dt*5);setEnemyFace(e,player.x-e.x);e.state='run';applyActorStairTerrain(e,s);return true;
  }
  ...
  function applyLowerTerrain(){
    ...
    if(nearClimb&&keys.up){beginPitClimbOut(b);return}
  }
  ...
  function applyPlayerTerrain(){
    if(player.z>0||player.vz!==0){
      // 抬腿帧的脚部越过平台面后，才允许横向跨出原平台并进入另一侧平台上空。
      const liftedFootY=player.y-player.z+8,clearanceReached=player.z>=44;if(clearanceReached)jumpPlatformCleared=true;
      const target=player.vz<0?currentPlatforms().filter(p=>p!==jumpSourcePlatform).map(p=>({p,landingY:clamp(player.y,p.minY+12,p.maxY-12)})).find(({p,landingY})=>insidePlatform(player.x,landingY,18,p)&&liftedFootY<=landingY):null;
      if(target){player.level=target.p.level;player.y=target.landingY;player.z=Math.max(0,player.y-liftedFootY+8);jumpSourcePlatform=target.p;return}
      if(!jumpPlatformCleared&&jumpSourcePlatform){const b=platformBoundsAtY(clamp(player.y,jumpSourcePlatform.minY,jumpSourcePlatform.maxY),jumpSourcePlatform);player.x=clamp(player.x,b.left+28,b.right-28)}
      else player.x=clamp(player.x,stageLockX,stageRightX);return
    }
    ...
    for(const c of currentCollapseClimbs()){
      ...
      if(near&&keys.up){beginCollapseClimb(c);return}
    }
  ...
  function grenadeBlast(p){
    ...
    for(const e of enemies){
      ...
      if(player.grab===e){e.grabbed=false;clearGrab();player.state='idle'}e.hp-=34;e.inv=.28;e.grabbed=false;e.state='thrown';e.timer=.9;e.throwHits=new Set();e.throwVx=(Math.sign(dx)||player.face)*(e.type==='heavy'?240:410)*(1-d/radius*.28);e.z=Math.max(24,e.z);e.vz=e.type==='heavy'?420:570;impact(e.x,e.y-e.z-65,34,false,e.throwVx);
    }
    message='炸开了！别让他站稳';messageT=1;
  }
  function hammerStun(p,e){
    const airZ=e.z||0,airVz=e.vz||0,airCarryX=e.throwVx||e.knockVx||0,wasAirborne=airZ>0||Math.abs(airVz)>1||e.airLaunch||e.state==='slamAir';e.hp-=16;e.inv=.25;e.x+=Math.sign(p.vx)*24;impact(e.x,e.y-airZ-78,16,false,p.vx);hitStop(.1);
  ...
  function grantEnemyReward(e){
    ...
    const gold=8+Math.floor(e.rank*1.6)+(e.elite?28:0),chicken=1+(e.rank%4===0?1:0)+(e.elite?2:0),fruitDrop=e.elite||Math.random()<Math.min(.18,.025+e.rank*.002);
    const drops=[{type:'coin',value:gold,dx:-24},{type:'chicken',value:chicken,dx:18}];if(fruitDrop)drops.push({type:'fruit',value:1,dx:55});
  ...
  function stageCleared(){return enemies.length>0&&enemies.every(e=>e.dead&&e.timer<=0)&&!pickups.some(p=>p.active&&['coin','chicken','fruit'].includes(p.type))}
  ...
  function advanceStage(){
    const map=trialMaps[mapIndex];
    if(mapIndex<trialMaps.length-1){mapIndex++;message='这张地图打穿了，换地方';messageT=1.6;loadMap(mapIndex);return}
    mapCycle++;loadMap(0);message=`第 ${mapCycle} 轮地图打穿了……重新洗牌，继续打`;messageT=2.2;return;
  }
  ...
  function update(dt){if(player.state==='climbOut'&&!player.climb){player.state='idle';player.z=0;player.vz=0;player.level=0;player.subPit=null;player.pitSafeT=1.1}if(player.climb){if((player.climb.kind==='pitOut'||player.climb.kind==='collapseUp')&&player.climb.t>=player.climb.dur+.08){player.climb.t=player.climb.dur}updateClimb(dt);updateCamera();return}if(slow>0){slow-=dt;return}player.inv=Math.max(0,player.inv-dt);player.risingInvT=Math.max(0,(player.risingInvT||0)-dt);if(player.risingInvT<=0||player.state!=='risingPunch')player.risingAirInv=false;player.pitSafeT=Math.max(0,(player.pitSafeT||0)-dt);player.comboT=Math.max(0,player.comboT-dt);player.kickComboT=Math.max(0,player.kickComboT-dt);player.risingCooldown=Math.max(0,player.risingCooldown-dt);messageT=Math.max(0,messageT-dt);flash=Math.max(0,flash-dt);screenTint=Math.max(0,screenTint-dt);if(powerSwitch)powerSwitch.flash=Math.max(0,powerSwitch.flash-dt);shake*=Math.pow(.04,dt);for(const f of hitFx)f.t-=dt;hitFx=hitFx.filter(f=>f.t>0);for(const f of blastFx)f.t-=dt;blastFx=blastFx.filter(f=>f.t>0);for(const f of resourceFx)f.t-=dt;resourceFx=resourceFx.filter(f=>f.t>0);updateProjectiles(dt);
    ...
    if(player.grab){const e=player.grab;player.grabT+=dt;player.grabEscapeT=Math.max(0,GRAB_MAX_DURATION-player.grabT);if(player.state==='drag')player.state='grab';e.x=player.x+player.face*47;e.y=player.y-2;e.z=player.z||0;e.vz=player.vz||0;setEnemyFace(e,-player.face);if(player.grabT>=GRAB_MAX_DURATION){releaseGrab(false,true);message='五秒到了……他挣开了！';messageT=1.1}}
      ...
      const sp=playerSpeedMul(),len=Math.hypot(dx,dy)||1;dx/=len;dy/=len;if(dx)player.face=Math.sign(dx);player.x+=dx*225*sp*dt;player.y+=dy*145*sp*dt;if(player.z===0&&player.vz<=0)player.state=(dx||dy)?'run':'idle';
    }
    ...
    if(player.z>0||player.vz>0){
      if(player.knockVx){player.x=clamp(player.x+player.knockVx*dt,stageLockX,stageRightX);player.knockVx*=Math.pow(.12,dt)}
      player.z+=player.vz*dt;player.vz-=1200*dt;
      if(player.z<=0){
        player.z=0;player.vz=0;player.knockVx=0;player.risingAirInv=false;player.risingInvT=0;
        ...
        if(player.fallDamage>0){
          const dmg=player.fallDamage;player.fallDamage=0;player.hp=Math.max(0,player.hp-dmg);impact(player.x,player.y-38,dmg,true,0);hitStop(.08);message=player.hp<=0?'摔狠了……站不起来':'腿麻了……疼得够呛 -'+dmg;messageT=player.hp<=0?99:1;
        ...
        }else if(player.state==='jump'||player.state==='airBackKick'||player.state==='risingPunch'){player.state='jumpLand';player.timer=.28;jumpSourcePlatform=currentPlatforms().find(p=>insidePlatform(player.x,player.y,18,p))||null;jumpPlatformCleared=false}
      }
    }
    for(const c of companions)updateCompanion(c,dt);
    ...
    if(!freeTourMode&&wave===1&&gatePhase==='combat'&&stageCleared()){if(gauntletMode)advanceGauntlet();else advanceStage()}updateCamera()
  }
  function heavySlamImpact(e){
    const radius=e.elite?175:145,damage=e.elite?38:26,dx=player.x-e.x,dy=(player.y-e.y)*1.35;
    impact(e.x,e.y-20,null,false,0);e.slamHit=true;
    if(player.z<55&&player.level===e.level&&Math.hypot(dx,dy)<radius){hurtPlayer(damage,Math.sign(dx||e.face)*(e.elite?38:30));message=e.elite?'地面都震了，别贴太近！':'这胖子砸下来太重了';messageT=1}
  }
  function enemyDmg(e,m=1){return Math.max(1,Math.round((e.damage||18)*m))}
  function startEnemySpecial(e,d){
    e.specialT=1.6+Math.random()*.9+Math.max(0,2.2-e.rank*.03);e.specialHit=false;e.specialDone=false;
    ...
    if(e.type==='grappler'&&d<145&&Math.abs(player.y-e.y)<64){e.state='enemyGrabWindup';e.timer=.38;message='他想抓我，不能让他贴身';messageT=.65;return true}
    ...
    if(e.type==='whip'&&d<470&&Math.abs(player.y-e.y)<76){e.state='whipWindup';e.timer=.22;message='别和她站一条线，鞭子太长';messageT=.55;return true}
    return false
  }
  function updateEnemySpecialState(e,dt){
    ...
    if(e.state==='barbarianRevive'){e.timer-=dt;e.inv=Math.max(e.inv,.12);if(e.timer<=0){e.revived=true;e.rage=true;e.maxHp=Math.max(38,Math.round(e.maxHp*.38));e.hp=e.maxHp;e.speed*=1.38;e.damage=Math.round(e.damage*1.18);setEnemyFace(e,player.x-e.x,true);e.state='barbarianUppercut';e.timer=.56;e.specialHit=false;e.inv=.65;e.specialT=1.2;message='荒铁蛮士复活——暴走挥棒！';messageT=1.35}return true}
    if(e.state==='barbarianCharge'){e.timer-=dt;const b=laneBoundsFor(e.level,e.y,50);e.x=clamp(e.x+e.face*(e.rage?430:350)*dt,b.left,b.right);e.y+=(player.y-e.y)*Math.min(1,dt*3);if(!e.specialHit&&player.level===e.level&&Math.abs(player.x-e.x)<78&&Math.abs(player.y-e.y)<55){e.specialHit=true;hurtPlayer(enemyDmg(e,.55),e.face*38)}if(e.timer<=0){e.state='barbarianUppercut';e.timer=.56;e.specialHit=false}return true}
    if(e.state==='barbarianUppercut'){e.timer-=dt;if(!e.specialHit&&e.timer<.31&&player.level===e.level&&Math.abs(player.x-e.x)<125&&Math.abs(player.y-e.y)<62){e.specialHit=true;knockPlayerDown(enemyDmg(e,1.1),e.face*92,{ignoreInv:true,launchVz:e.rage?820:720});message='狼牙棒从下往上掀飞了！';messageT=.7}if(e.timer<=0)e.state='idle';return true}
    if(e.state==='spinAir'){e.timer-=dt;const b=laneBoundsFor(e.level,e.y,45);e.x=clamp(e.x+(e.spinVx||0)*dt,b.left,b.right);e.z=55+Math.sin((1-e.timer/.78)*Math.PI)*45;if(!e.specialHit&&player.level===e.level&&zReach(e,player,145)&&Math.abs(player.x-e.x)<86&&Math.abs(player.y-e.y)<58){e.specialHit=true;knockPlayerDown(enemyDmg(e,1.05),e.face*68)}if(e.timer<=0){e.z=0;e.state='idle'}return true}
    if(e.state==='enemyGrabWindup'){e.timer-=dt;e.y+=(player.y-e.y)*Math.min(1,dt*8);if(e.timer<=0){const caught=player.level===e.level&&player.hp>0&&player.z<70&&Math.abs(player.x-e.x)<110&&Math.abs(player.y-e.y)<55;if(caught){e.state='enemyGrabbed';e.timer=.8;e.specialHit=false}else e.state='idle'}return true}
    if(e.state==='enemyGrabbed'){e.timer-=dt;if(player.state!=='enemyGrabbed')player.face=-e.face;player.state='enemyGrabbed';player.timer=Math.max(player.timer,e.timer+.1);player.x=e.x+e.face*40;if(e.timer<=0){e.state='enemyThrow';e.timer=.6;e.specialHit=false}return true}
    if(e.state==='enemyThrow'){e.timer-=dt;if(!e.specialHit&&player.level===e.level&&zReach(e,player,180)&&Math.abs(player.x-e.x)<94&&Math.abs(player.y-e.y)<60){e.specialHit=true;knockPlayerDown(enemyDmg(e,1.18),e.face*130,{ignoreInv:true,breakHold:true,launchVz:740});message='可恶，被他借力甩了！';messageT=.75}if(e.timer<=0)e.state='idle';return true}
    ...
    if(e.state==='axeSlash'){const prev=e.timer;e.timer-=dt;const elapsed=.78-e.timer,hitTimes=[.12,.34,.56];for(let i=0;i<hitTimes.length;i++){if(!e.axeHits?.[i]&&prev>.78-hitTimes[i]&&elapsed>=hitTimes[i]){if(!e.axeHits)e.axeHits=[false,false,false];e.axeHits[i]=true;if(player.level===e.level&&zReach(e,player,130)&&Math.abs(player.x-e.x)<112&&Math.abs(player.y-e.y)<58){hurtPlayer(enemyDmg(e,.55),e.face*(34+i*10),{ignoreInv:true,inv:.09,timer:.24,hitStop:.055});message=`斧王第 ${i+1} 劈！`;messageT=.35}}}if(e.timer<=0)e.state='idle';return true}
    if(e.state==='teleport'){e.timer-=dt;if(!e.specialDone&&e.timer<.17){e.specialDone=true;e.x=clamp(player.x-player.face*78,70,WORLD_W-70);e.y=player.y;e.level=player.level;setEnemyFace(e,player.face,true);e.state='stab';e.timer=.32;e.specialHit=false}return true}
    if(e.state==='stab'){e.timer-=dt;if(!e.specialHit&&player.level===e.level&&zReach(e,player,120)&&Math.abs(player.x-e.x)<92&&Math.abs(player.y-e.y)<52){e.specialHit=true;hurtPlayer(enemyDmg(e,1.25),e.face*48);message='背后一凉……中刀了';messageT=.65}if(e.timer<=0)e.state='idle';return true}
    if(e.state==='suitCombo'){const prev=e.timer;e.timer-=dt;const elapsed=.66-e.timer,hitTimes=[.14,.3,.46];for(let i=0;i<hitTimes.length;i++){if(!e.suitHits?.[i]&&prev>.66-hitTimes[i]&&elapsed>=hitTimes[i]){if(!e.suitHits)e.suitHits=[false,false,false];e.suitHits[i]=true;if(player.level===e.level&&zReach(e,player,125)&&Math.abs(player.x-e.x)<118&&Math.abs(player.y-e.y)<54)hurtPlayer(enemyDmg(e,.38),e.face*(18+i*8))}}if(e.timer<=0)e.state='idle';return true}
    if(e.state==='whipWindup'){e.timer-=dt;if(e.timer<=0){e.state='whipStrike';e.timer=.36;e.specialHit=false}return true}
    if(e.state==='whipStrike'){e.timer-=dt;const dx=(player.x-e.x)*e.face;if(!e.specialHit&&player.level===e.level&&zReach(e,player,160)&&dx>45&&dx<340&&Math.abs(player.y-e.y)<46){e.specialHit=true;knockPlayerDown(enemyDmg(e,1.05),e.face*84);message='鞭子抽得太远了，别站直线';messageT=.8}if(e.timer<=0)e.state='idle';return true}
    return false
  }
  const enemyAttackStates=new Set(['slamCharge','slamAir','slamLand','slideWindup','slide','spinAir','enemyGrabWindup','enemyGrabbed','enemyThrow','axeWindup','axeSlash','teleport','stab','suitCombo','whipWindup','whipStrike','lightsOutCast','barbarianRevive','barbarianCharge','barbarianUppercut','breakerCurrent']);
  ...
  function updateEnemy(e,dt){e.inv=Math.max(0,e.inv-dt);e.faceHold=Math.max(0,(e.faceHold||0)-dt);if(e.state==='knockdown'){const b=laneBoundsFor(e.level,e.y,35);e.timer-=dt;e.x=clamp(e.x+e.knockVx*dt,b.left,b.right);e.knockVx*=Math.pow(.07,dt);if(e.airLaunch){e.z+=e.vz*dt;e.vz-=1450*dt;if(catchEnemyOnPlatform(e))return;if(e.z<=0&&e.vz<0){for(const pit of currentPits())if(insidePit(e,pit))return dropEnemyIntoPit(e,pit);e.z=0;e.vz=0;e.airLaunch=false;e.timer=Math.min(e.timer,.14);impact(e.x,e.y-18,null,false,e.knockVx)}}if(e.timer<=0&&!e.airLaunch){e.state='down';e.timer=e.dead?3:2.2;e.knockVx=0}return}if(e.state==='thrown'){const b=laneBoundsFor(e.level,e.y,35);e.timer-=dt;e.x=clamp(e.x+e.throwVx*dt,b.left,b.right);e.throwVx*=Math.pow(.22,dt);e.z+=e.vz*dt;e.vz-=1450*dt;if(catchEnemyOnPlatform(e))return;for(const other of enemies){if(other===e||other.dead||other.grabbed||other.level!==e.level||e.throwHits?.has(other))continue;if(Math.abs(other.x-e.x)<82&&Math.abs(other.y-e.y)<50){e.throwHits?.add(other);hurtEnemy(other,22,Math.sign(e.throwVx||-player.face)*310);impact(other.x,other.y-75,22,false,e.throwVx);message='砸中了！身后也不安全';messageT=.85}}if(e.z<=0&&e.vz<0){for(const pit of currentPits())if(insidePit(e,pit))return dropEnemyIntoPit(e,pit);e.z=0;e.vz=0;e.throwVx=0;e.state='down';e.timer=1.05;impact(e.x,e.y-18,34,false,-player.face*220);if(e.hp<=0&&!e.dead){e.dead=true;player.kills++}}return}if(e.dead){if(e.timer>0)e.timer-=dt;return}e.attackT=Math.max(0,e.attackT-dt);e.slamT=Math.max(0,e.slamT-dt);if(e.grabbed)return;
    ...
    if(e.type==='breaker'&&powerSwitch&&powerSwitch.on&&powerSwitch.level===e.level){
      ...
      const len=Math.hypot(sx,sy)||1,b=laneBoundsFor(e.level,e.y,42);setEnemyFace(e,sx);e.x=clamp(e.x+sx/len*e.speed*dt,b.left,b.right);e.y+=sy/len*e.speed*.68*dt;e.state='run';return
    }
    if(player.fallFromLevel!=null&&player.z>35){if(e.state==='slamCharge'||e.state==='slideWindup')e.state='idle';setEnemyFace(e,player.x-e.x);return}
    if(updateEnemySpecialState(e,dt))return;
    if(e.state==='slamCharge'){e.timer-=dt;e.slamTargetX=clamp(player.x,e.x-95,e.x+95);if(e.timer<=0){e.state='slamAir';e.timer=1.15;e.z=1;e.vz=e.elite?650:610;e.slamHit=false}return}
    ...
    if(e.state==='slide'){
      ...
      e.y+=(player.y-e.y)*Math.min(1,dt*2.6);
      ...
      if(!e.slideHit&&e.level===player.level&&zReach(e,player,58)&&Math.abs(player.x-e.x)<76&&Math.abs(player.y-e.y)<48){e.slideHit=true;knockPlayerDown(16,e.face*34)}
    ...
    const senseX=Math.abs(player.x-e.x),senseY=Math.abs(player.y-e.y),aggro=e.awake||(Math.abs(player.level-e.level)<=2&&senseX<900&&senseY<360);
    if(!aggro){e.state='idle';setEnemyFace(e,player.x-e.x);return}
    e.awake=true;
    if(e.level!==player.level){enemyUseStairs(e,dt);return}
    const d=dist(e,player),dx=player.x-e.x,dy=player.y-e.y;setEnemyFace(e,dx);
    const facingTarget=!Math.sign(dx)||Math.sign(dx)===e.face;
    const anotherSlamming=enemies.some(o=>o!==e&&!o.dead&&o.level===e.level&&o.state.startsWith('slam')),canAttack=activeEnemyAttacks()<3&&facingTarget;
    if(e.specialT<=0&&canAttack&&startEnemySpecial(e,d))return;
    if(e.type==='heavy'&&e.slamT<=0&&d<225&&!anotherSlamming&&canAttack){e.state='slamCharge';e.timer=e.elite?.62:.72;e.slamTargetX=player.x;e.attackT=1.3;e.slamT=e.elite?2.8:3.7;message=e.elite?'他要砸地，离远点！':'胖子蹲下了……要跳了';messageT=.8;return}
    if(e.type==='skinny'&&e.slideT<=0&&d<310&&canAttack){e.state='slideWindup';e.timer=.24;e.slideT=2.1+Math.random()*.8;message='他压低身体了……小心下段';messageT=.55;return}
    if(e.type==='whip'){
    ...
    const p=platformForLevel(e.level);if(e.type==='skinny'&&p&&e.state==='hurt'&&!insidePlatform(e.x,e.y,0,p)){e.level=0;e.x=clamp(e.x,55,WORLD_W-55);e.y=570;e.hp-=18;e.state='down';e.timer=1;message='摔下去了……这高度够他受';messageT=1.1;if(e.hp<=0)e.dead=true}
  }
  ...
  function drawThemeOverlay(){
    ...
    if(t==='沙漠'){
      ...
      for(let x=-520;x<WORLD_W;x+=980){
        ...
        g.strokeStyle='#2b332eaa';g.lineWidth=3;for(let i=0;i<18;i++){const bx=x+150+i*42,by=376+Math.sin((i+x)*.24)*26;g.beginPath();g.moveTo(bx,by-18);g.lineTo(bx+16,by+14);g.stroke()}
      }
  ...
  function drawScene(){
    ...
    const vignette=g.createRadialGradient(cameraX+W*.5,cameraY+H*.48,150,cameraX+W*.5,cameraY+H*.5,760);vignette.addColorStop(0,'#00000008');vignette.addColorStop(.62,'#00000022');vignette.addColorStop(1,'#000000b8');g.fillStyle=vignette;g.fillRect(cameraX,cameraY,W,H);g.fillStyle='#05070818';for(let y=cameraY+1;y<cameraY+H;y+=4)g.fillRect(cameraX,y,W,1);
  }
  ...
  function drawUpperLayerMist(){
    ...
    const fogBottom=GROUND_Y+104,visibleBottom=Math.min(fogBottom,cameraY+H);if(visibleBottom<=cameraY)return;
    g.save();
    const fade=92,top=cameraY,bottom=visibleBottom;
  ...
  function drawPowerSwitch(){
    ...
    g.save();g.translate(p.x,p.y);drawGroundShadow(34,8,5,'#03050699');
    g.fillStyle='#171d1f';g.fillRect(-31,-108,62,102);g.strokeStyle=p.on?'#d9c36a':'#557078';g.lineWidth=5;g.strokeRect(-28,-105,56,96);
    g.fillStyle=p.on?'#d6b94b':'#243438';g.fillRect(-19,-91,38,52);g.fillStyle=p.on?'#f2d66d':'#91a1a4';g.fillRect(-8,p.on?-77:-61,16,30);
    g.strokeStyle=p.on?'#f6dc7a':'#79cbe0';g.lineWidth=4;g.beginPath();g.moveTo(0,p.on?-62:-46);g.lineTo(p.on?12:-12,p.on?-45:-76);g.stroke();
    g.fillStyle=p.on?'#80d675':'#b44b3d';g.beginPath();g.arc(0,-20,8+(p.flash>0?Math.sin(t)*3:0),0,6.3);g.fill();
  ...
  function drawModule(m){g.save();g.translate(m.x,m.y);switch(m.type){
    ...
    if(materialAtlas.complete&&materialAtlas.naturalWidth){let panel=-1,alpha=.2;if(m.type==='mud'){panel=0;alpha=.34}else if(m.type==='wall'){panel=1;alpha=.2}else if(m.type==='platform'||m.type==='elevator'){panel=2;alpha=.28}if(panel>=0){g.globalAlpha=alpha;g.globalCompositeOperation='soft-light';g.drawImage(materialAtlas,panel*256,0,256,256,0,0,m.w,m.h);g.globalCompositeOperation='source-over';g.globalAlpha=1}}
  }g.restore()}
  function lamp(x,y){g.fillStyle=palette.ink;g.fillRect(x-16,y-20,32,55);g.fillStyle='#f2b963';g.fillRect(x-9,y-12,18,27);g.fillStyle='#e6a54a33';g.beginPath();g.moveTo(x-9,y+15);g.lineTo(x-80,y+140);g.lineTo(x+80,y+140);g.lineTo(x+9,y+15);g.fill()}
  function crate(x,y,w,h){g.fillStyle=palette.ink;g.fillRect(x-5,y-5,w+10,h+10);g.fillStyle='#6f4a32';g.fillRect(x,y,w,h);g.strokeStyle='#a46941';g.lineWidth=5;g.strokeRect(x+4,y+4,w-8,h-8);g.beginPath();g.moveTo(x+6,y+6);g.lineTo(x+w-6,y+h-6);g.moveTo(x+w-6,y+6);g.lineTo(x+6,y+h-6);g.stroke()}
  function warningSign(x,y){g.fillStyle='#c57436';g.fillRect(x,y,74,42);g.fillStyle=palette.ink;g.beginPath();g.moveTo(x+37,y+7);g.lineTo(x+63,y+35);g.lineTo(x+11,y+35);g.closePath();g.fill();g.strokeStyle='#f1c07a';g.lineWidth=4;g.beginPath();g.moveTo(x+37,y+15);g.lineTo(x+37,y+27);g.moveTo(x+37,y+32);g.lineTo(x+37,y+34);g.stroke()}
  ...
  function render(){updateCamera();g.save();g.translate(-cameraX+(Math.random()-.5)*shake,-cameraY+(Math.random()-.5)*shake);drawScene();
    ...
    effects();g.restore();hud()}
  function drawFighter(a,isP){
    const x=a.x,y=a.y-a.z,face=a.face||1,state=a.state,phase=performance.now()/105;
    if(isP&&heroKickSheet.complete&&heroKickSheet.naturalWidth){drawHeroSprite(a,x,y,face,state,phase);return}
  ...
  function drawPitClimbOutSprite(a,x,y,face){
    ...
    const sw=heroClimbOutSheet.naturalWidth/6,sh=heroClimbOutSheet.naturalHeight,scale=.37*enemyFrameScale('heroKick3',frame,1),dw=sw*scale,dh=sh*scale;
  ...
  function drawHeroSprite(a,x,y,face,state,phase){
    if(state==='run'){if(heroWalkSheet.complete&&heroWalkSheet.naturalWidth){drawWalkSprite(a,x,y,face,phase);return}drawMovementSprite(a,x,y,face,state,phase,false);return}
    if(state==='climb'&&heroWalkSheet.complete&&heroWalkSheet.naturalWidth){drawWalkSprite(a,x,y,face,phase);return}
    ...
    const paintShadow=(sheet,count,frame,scale,alpha=1,tx=0,sx=1)=>{const shadow=fighterShadowMap.get(sheet);if(!shadow?.complete||!shadow.naturalWidth)return;scale=characterFrameScale(sheet,frame);const sw=shadow.naturalWidth/count,sh=shadow.naturalHeight,dw=sw*scale,dh=sh*scale;g.save();g.globalAlpha=alpha;g.filter='none';g.translate(tx,a.z-groundFix);g.scale(sx,1);g.drawImage(shadow,frame*sw,0,sw,sh,-dw*.5,-dh+40*scale,dw,dh);g.restore()};
    const paint=(sheet,frame,scale,alpha=1,tx=0,ty=0,rot=0,sx=1,filter='none')=>{scale=characterFrameScale(sheet,frame);const sw=sheet.naturalWidth/3,sh=sheet.naturalHeight,dw=sw*scale,dh=sh*scale;paintShadow(sheet,3,frame,scale,alpha,tx,sx);g.save();g.globalAlpha=alpha;g.filter=filter;g.translate(tx,ty);g.rotate(rot);g.scale(sx,1);g.drawImage(sheet,frame*sw,0,sw,sh,-dw*.5,-dh+40*scale,dw,dh);g.restore()};
    const paintN=(sheet,count,frame,scale,tx=0,ty=0,sx=1,filter='none')=>{scale=characterFrameScale(sheet,frame);const sw=sheet.naturalWidth/count,sh=sheet.naturalHeight,dw=sw*scale,dh=sh*scale;paintShadow(sheet,count,frame,scale,1,tx,sx);g.save();g.filter=filter;g.translate(tx,ty);g.scale(sx,1);g.drawImage(sheet,frame*sw,0,sw,sh,-dw*.5,-dh+40*scale,dw,dh);g.restore()};
    ...
    if(state.startsWith('punch')&&heroPunchComboSheet.complete){
      ...
      const p=clamp(1-a.timer/.62,0,.9999),frames=[1,2,3,4],frame=frames[Math.min(frames.length-1,Math.floor(p*frames.length))];paintN(heroJumpKickSheet,6,frame,.41,0,0,1,'none');
  ...
  function drawWalkSprite(a,x,y,face,phase){
    const frame=Math.floor(phase/1.5)%7,sw=heroWalkSheet.naturalWidth/7,sh=heroWalkSheet.naturalHeight,scale=enemyFrameScale('heroWalk',frame,1),dw=sw*scale,dh=sh*scale,frameShifts=[0,0,0,0,0,0,0];
  ...
  function drawMovementSprite(a,x,y,face,state,phase,isEnemy){
    const sw=heroKickSheet.naturalWidth/3,sh=heroKickSheet.naturalHeight,scale=enemyFrameScale('heroIdle',0,1),dw=sw*scale,dh=sh*scale;
    const shadowSheet=fighterShadows.heroKickSheet;if(shadowSheet?.complete&&shadowSheet.naturalWidth){g.save();g.translate(x,y);g.scale(face,1);g.drawImage(shadowSheet,0,0,sw,sh,-dw*.5,-dh+40,dw,dh);g.restore()}
    g.save();g.translate(x,y);g.scale(face,1);
    if(isEnemy)g.filter=a.elite?'grayscale(.35) sepia(.75) hue-rotate(350deg) brightness(.72) contrast(1.2)':'grayscale(.7) sepia(.4) hue-rotate(125deg) brightness(.58) contrast(1.25)';
    else g.filter='none';
  ...
  function generatedEnemyFrame(a,state,phase){
    ...
    if(state==='hurt'||state==='grabbed'||state==='stunned'||state==='guard'||state==='thrown'||(state==='knockdown'&&a.airLaunch))return 3;
    ...
    const attackStates=['attack','slamCharge','slamAir','slamLand','slideWindup','slide','spinAir','enemyGrabWindup','enemyGrabbed','enemyThrow','axeWindup','axeSlash','stab','vanish','suitCombo','lightsOutCast','whipWindup','whipStrike','barbarianCharge','barbarianUppercut','breakerCurrent'];
  ...
  const defaultEnemyFrameScales={"heroIdle":[1,1,1],"heroWalk":[1,1,1,1,1,1,1],"heroCombat":[1,1,1],"heroPunch":[1,1,1,1,1,1],"heroHurt":[1,1,1,1,1],"heroKnockdown":[1,1,1,1,1],"heroJumpTransition":[1,1],"heroBackThrow1":[1],"heroBackThrow2":[1],"heroBackThrow3":[1],"heroBackThrow4":[1],"heroGrab":[1,1,1],"heroGrabKnee":[1,1,1],"heroKick1":[1,1,1,1],"heroKick2":[1,1,1,1,1],"heroKick3":[1,1,1,1,1,1],"heroJumpKick":[1,1,1,1,1,1],"heroRisingPunch":[1,1,1,1],"skinnyBase":[1,1,1,1,1.16],"skinnyWalk":[1,1,1,1],"skinnySlide":[1],"heavyBase":[1,1,1,1,1,1],"heavyWalk":[1,1,1,1],"heavyDeath":[1],"spinnerBase":[0.86,0.86,0.86,0.86,0.86],"spinnerSpin":[0.66,0.57,0.67,0.77],"grapplerBase":[1,1,1,1,1],"axeBase":[1,1,1,1,1],"assassinBase":[1,1,1,1,1],"suitBase":[1,1,1,1,1],"breakerBase":[1,1,1,1,1],"whipBase":[1,1,1,1,1],"barbarianBase":[0.95,0.86,1.22,0.88,1.1],"barbarianRevive":[1,0.98,1.06,1.04]};
  ...
  function drawAxeChopLayer(state,phase,timer){
    const windup=state==='axeWindup'?clamp(1-timer/.48,0,1):1;
    ...
    if(state==='axeWindup'){
      const glow=.24+.36*windup+.12*Math.sin(phase*2.2);
      g.globalAlpha=glow;
      const grad=g.createLinearGradient(-32,-214,70,-96);
      grad.addColorStop(0,'#ffffff00');
      grad.addColorStop(.45,'#ffe6a8dd');
      grad.addColorStop(1,'#ff8a2a00');
      g.strokeStyle=grad;
      g.lineWidth=10;
      g.beginPath();
      g.moveTo(-26,-206);
      g.bezierCurveTo(2,-196,48,-154,66,-98);
      g.stroke();
      g.globalAlpha=.32*windup;
      g.strokeStyle='#ffb347aa';
      g.lineWidth=3;
      g.beginPath();
      g.arc(14,-166,44,-2.2,.42);
      g.stroke();
    }else if(state==='axeSlash'){
      const comboIndex=Math.min(2,Math.floor(slashTotal*3));
      const flash=Math.max(0,1-Math.abs(slash-.32)/.34);
      const after=Math.max(0,1-slash);
      const laneShift=(comboIndex-1)*24;
      const xTop=-42+slash*86+laneShift,xBottom=54-slash*42+laneShift*.45;
      const cutGrad=g.createLinearGradient(xTop,-222,xBottom,-16);
      cutGrad.addColorStop(0,'#ffffff00');
      cutGrad.addColorStop(.2,'#fff7d6ee');
      cutGrad.addColorStop(.55,'#ffcf73dd');
      cutGrad.addColorStop(1,'#ff762800');
      g.globalAlpha=.35+flash*.55;
      g.strokeStyle='#ff8f2f66';
      g.lineWidth=34-8*slash;
      g.beginPath();
      g.moveTo(xTop,-216);
      g.bezierCurveTo(22,-172,62,-96,xBottom,-20);
      g.stroke();
      g.globalAlpha=.5+flash*.45;
      g.strokeStyle=cutGrad;
      g.lineWidth=15-4*slash;
      g.beginPath();
      g.moveTo(xTop,-216);
      g.bezierCurveTo(18,-168,58,-96,xBottom,-20);
      g.stroke();
      g.globalAlpha=.82*flash;
      g.strokeStyle='#fff9db';
      g.lineWidth=3.2;
      g.beginPath();
      g.moveTo(xTop+5,-208);
      g.bezierCurveTo(28,-151,54,-84,xBottom+4,-25);
      g.stroke();
      g.globalCompositeOperation='source-over';
      g.globalAlpha=.34*after+.22;
      g.fillStyle='#c8864a66';
      g.beginPath();
      g.ellipse(70,-6,58+slash*18,10+flash*5,0,0,Math.PI*2);
      g.fill();
      g.strokeStyle='#f3bf6c88';
      g.lineWidth=2.4;
      g.beginPath();
      g.moveTo(20,-12);
      g.lineTo(122+slash*18,-18);
      g.moveTo(30,-3);
      g.lineTo(104+slash*12,3);
      g.stroke();
    }
    g.restore();
  }
  function drawSpecialEnemyStrip(a,sheet,shadowSheet,count,frame,x,y,face,{air=false,rage=false,scaleKey=null}={}){
    ...
    if(shadowSheet?.complete&&shadowSheet.naturalWidth){g.save();g.translate(x,y+(a.z||0));g.scale(face,1);g.drawImage(shadowSheet,frame*sw,0,sw,sh,drawX,drawY,dw,dh);g.restore()}
    g.save();g.translate(x,y-(air?Math.max(22,a.z||0):0));g.scale(face,1);
    if(rage){g.save();g.globalCompositeOperation='screen';g.globalAlpha=.25+.12*Math.sin(performance.now()/70);g.strokeStyle='#ff5a24';g.lineWidth=10;for(let i=0;i<3;i++){g.beginPath();g.arc(0,-112,62+i*13,-2.65+i*.15,.5+i*.1);g.stroke()}g.restore()}
  ...
  function drawGeneratedEnemySprite(a,sheet,x,y,face,state,phase){
    ...
    if(a.type==='barbarian'&&state==='barbarianRevive'&&barbarianReviveSheet.complete&&barbarianReviveSheet.naturalWidth){
      ...
      drawSpecialEnemyStrip(a,barbarianReviveSheet,fighterShadows.barbarianReviveSheet,4,frame,x,y,face,{rage:true,scaleKey:'barbarianRevive'});
      drawEnemyBar(a,x,y-225*(a.elite?1.08:1));return
    }
    if(a.type==='barbarian'&&state==='barbarianCharge'&&barbarianReviveSheet.complete&&barbarianReviveSheet.naturalWidth){
      // The charge pose is the fourth frame of the revive strip (index 3).
    ...
    if(state==='thrown'){
      ...
      g.rotate(dir*(-.2+.08*Math.sin(p*Math.PI)));
    }
    g.filter=a.elite?'brightness(.93) saturate(1.22) contrast(1.16) drop-shadow(0 0 5px #9b4b2b88)':'brightness(.96) saturate(1.06) contrast(1.08)';
    ...
    if(a.type==='axe'&&(state==='axeWindup'||state==='axeSlash')){
      ...
      g.save();g.globalCompositeOperation='screen';g.lineCap='round';
      for(let pass=0;pass<3;pass++){g.strokeStyle=pass===2?'#ffffff':pass===1?'#66eaff':'#168dff88';g.lineWidth=pass===2?1.5:pass===1?4:10;g.beginPath();g.moveTo(0,-166);for(let yy=-150;yy<=250;yy+=18)g.lineTo((Math.sin(yy*.37+phase*3+pass)*13),yy);g.stroke()}
      g.fillStyle='#9ff8ff';for(let i=0;i<9;i++)g.fillRect(Math.sin(i*8+phase)*22,-150+i*43,3,3);
      g.restore();
  ...
  function drawEnemySprite(a,x,y,face,state,phase){
    ...
    if(generatedSheet?.complete&&generatedSheet.naturalWidth){drawGeneratedEnemySprite(a,generatedSheet,x,y,face,state,phase);return}
    if(a.type==='heavy'&&heavyEnemySheet.complete&&heavyEnemySheet.naturalWidth){drawHeavyEnemySprite(a,x,y,face,state,phase);if(state==='stunned')drawStunIndicator(x,y-172);return}
    if(a.type==='skinny'&&skinnyEnemySheet.complete&&skinnyEnemySheet.naturalWidth){drawSkinnyEnemySprite(a,x,y,face,state,phase);if(state==='stunned')drawStunIndicator(x,y-148);return}
    return
  }
  function drawSkinnyEnemySprite(a,x,y,face,state,phase){
    if(state==='run'&&skinnyEnemyWalkSheet.complete&&skinnyEnemyWalkSheet.naturalWidth){const walkFrame=Math.floor(phase/1.9)%4,scale=enemyFrameScale('skinnyWalk',walkFrame,1);drawEnemyWalkSprite(a,x,y,face,phase,skinnyEnemyWalkSheet,scale,[0,0,0,0],40,enemyVisualHeights.skinny);return}
    ...
    const scale=enemyFrameScale(scaleKey,frame,1),drawW=sheetW*scale,drawH=sheetH*scale;
    const groundFix=0;
    const shadowSheet=fighterShadowMap.get(sheet);if(shadowSheet?.complete&&shadowSheet.naturalWidth){g.save();g.translate(x,y+a.z+groundFix);g.scale(face,1);g.drawImage(shadowSheet,frame*sheetW,0,sheetW,sheetH,-drawW*.5,-drawH+40*scale,drawW,drawH);g.restore()}
    g.save();g.translate(x,y+groundFix);g.scale(face,1);g.translate(tx,ty);g.rotate(rot);if(state==='run')g.translate(Math.sin(phase*.95)*3,0);g.drawImage(sheet,frame*sheetW,0,sheetW,sheetH,-drawW*.5,-drawH+40*scale,drawW,drawH);g.restore();
    if(!a.dead)drawEnemyBar(a,x,y-160)
  }
  function drawHeavyEnemySprite(a,x,y,face,state,phase){
    if(state==='run'&&heavyEnemyWalkSheet.complete&&heavyEnemyWalkSheet.naturalWidth){const walkFrame=Math.floor(phase/1.9)%4,walkScale=enemyFrameScale('heavyWalk',walkFrame,1)*(a.elite?1.1:1);drawEnemyWalkSprite(a,x,y,face,phase,heavyEnemyWalkSheet,walkScale,[0,0,0,0],34,enemyVisualHeights.heavy*(a.elite?1.1:1));return}
    if(state==='heavyDefeated'&&heavyEnemyDeathSheet.complete&&heavyEnemyDeathSheet.naturalWidth){
      const size=384*enemyFrameScale('heavyDeath',0,1)*(a.elite?1.1:1),shadowSheet=fighterShadowMap.get(heavyEnemyDeathSheet);
      if(shadowSheet?.complete&&shadowSheet.naturalWidth){g.save();g.translate(x,y+a.z);g.scale(face,1);g.drawImage(shadowSheet,-size*.5,-size+40*(a.elite?1.1:1),size,size);g.restore()}
      g.save();g.translate(x,y);g.scale(face,1);if(a.elite)g.filter='sepia(.28) saturate(1.35) hue-rotate(330deg) brightness(.92) contrast(1.08)';g.drawImage(heavyEnemyDeathSheet,-size*.5,-size+40*(a.elite?1.1:1),size,size);g.filter='none';g.restore();return
    }
    ...
    else if(state==='hurt'||state==='grabbed'){frame=3;rot=-.08;tx=-5;ty=8}
    else if(state==='thrown'){const p=1-clamp(a.timer/.82,0,1),dir=Math.sign(a.throwVx||face)||1;frame=3;rot=dir*(-.16+.08*Math.sin(p*Math.PI));tx=-dir*7;ty=-2+Math.sin(p*Math.PI)*4;shadowW=72}
    else if(state==='knockdown'){frame=0;rot=-clamp(1-a.timer/(a.knockdownDuration||.82),0,1)*1.35;tx=18;ty=24;shadowW=76}
    else if(state==='heavyDefeated'){frame=5;ty=12;shadowW=72}
    else if(state==='down'){frame=5;rot=-1.18;tx=28;ty=33;shadowW=92}
    const scale=enemyFrameScale('heavyBase',frame,1)*(a.elite?1.1:1),dw=sw*scale,dh=sh*scale;
    if(state==='slamCharge'){
    ...
    const groundFix=0;
    const shadowSheet=fighterShadowMap.get(heavyEnemySheet);if(shadowSheet?.complete&&shadowSheet.naturalWidth){g.save();g.translate(x,y+a.z+groundFix);g.scale(face,1);g.drawImage(shadowSheet,frame*sw,0,sw,sh,-dw*.5,-dh+40*scale,dw,dh);g.restore()}
    g.save();g.translate(x,y+groundFix);g.scale(face,1);g.translate(tx,ty);g.rotate(rot);
    if(state==='run')g.translate(Math.sin(phase*.8)*2,0);
    if(state==='slamLand')g.scale(1.06,.94);
    if(a.elite)g.filter='sepia(.28) saturate(1.35) hue-rotate(330deg) brightness(.92) contrast(1.08)';
  ...
  function drawEnemyWalkSprite(a,x,y,face,phase,sheet,scale,frameShifts,bottomPad,labelHeight){
    const frame=Math.floor(phase/1.9)%4,sw=sheet.naturalWidth/4,sh=sheet.naturalHeight,dw=sw*scale,dh=sh*scale;
    const shadowSheet=fighterShadowMap.get(sheet);if(shadowSheet?.complete&&shadowSheet.naturalWidth){g.save();g.translate(x,y);g.scale(face,1);g.drawImage(shadowSheet,frame*sw,0,sw,sh,-dw*.5+frameShifts[frame],-dh+40*scale,dw,dh);g.restore()}
    g.save();g.translate(x,y);g.scale(face,1);
    if(a.elite)g.filter='sepia(.28) saturate(1.35) hue-rotate(330deg) brightness(.92) contrast(1.08)';
  ...
  function drawEnemyBar(a,x,labelY){g.save();bar(x-38,labelY-8,76,6,a.hp/a.maxHp,a.companion?'#3fa768':a.elite?'#bf7b2f':'#8f2c2b');g.fillStyle=a.companion?'#a9efbf':'#c9c3b7';g.font='12px sans-serif';g.textAlign='center';g.fillText(a.name,x,labelY-14);g.restore()}
  ...
  function hud(){g.fillStyle='#091014dd';g.fillRect(28,24,460,106);g.strokeStyle='#8a5638';g.strokeRect(28,24,460,106);g.fillStyle='#e8d5b3';g.font='800 21px sans-serif';g.textAlign='left';g.fillText('陆骁',48,54);bar(48,67,270,15,player.hp/player.maxHp,'#c84932');g.fillStyle='#96a5a5';g.font='14px sans-serif';g.fillText(`HP ${Math.max(0,player.hp)} / ${player.maxHp}`,328,80);g.fillStyle='#d5a467';g.fillText(player.held?(player.held==='grenade'?'手持：手榴弹':`手持：战锤（剩 ${player.heldUses} 次）`):'徒手格斗',48,94);
    ...
    const growthValues={hp:`${progress.hpLv}级 · HP ${playerMaxHp()}`,atk:`${progress.atkLv}级 · ×${playerAtkMul().toFixed(2)}`,def:`${progress.defLv}级 · 减伤${Math.round((1-playerDefMul())*100)}%`,spd:`${progress.spdLv}/5 · 进阶${progress.ascend}`};for(const [kind,value] of Object.entries(growthValues)){const node=document.querySelector(`#growth-${kind}`);if(node)node.textContent=value}
    g.fillStyle='#d8c39a';g.font='13px sans-serif';g.fillText(`金币 ${progress.gold}  鸡腿 ${progress.chicken}  果实 ${progress.fruit}`,48,113);g.fillStyle='#91a0a1';g.fillText(`1血${progress.hpLv} 2攻${progress.atkLv} 3防${progress.defLv} 4速${progress.spdLv}/5  进阶${progress.ascend}`,248,113);
    const place=levelName(player.level),map=trialMaps[mapIndex];g.textAlign='right';g.fillStyle='#ddd';g.font='700 17px sans-serif';if(freeTourMode)g.fillText(`自由游览 · 地图 ${mapIndex+1} / ${trialMaps.length}`,1240,49);else if(gauntletMode)g.fillText(`测试轮战 ${Math.min(gauntletIndex+1,gauntletEnemyTypes.length)} / ${gauntletEnemyTypes.length}`,1240,49);else g.fillText(`地图 ${mapIndex+1} / ${trialMaps.length} · 路段 ${gateIndex+1} / ${map.gates.length}`,1240,49);g.fillStyle='#91a0a1';g.font='14px sans-serif';g.fillText(`${gauntletMode?'模型测试场':map.name} · ${themeName(currentStageTheme)} · ${place}`,1240,72);g.fillText(freeTourMode?`队友 ${companions.length} · 敌人关闭`:`击倒 ${player.kills} · 队友 ${companions.filter(c=>!c.dead).length}/${companions.length}`,1240,94);
  ...
  function loop(now){const dt=Math.min(.033,(now-last)/1000||0);last=now;if(running)update(dt);render();requestAnimationFrame(loop)}
  function key(e,on){const m={ArrowLeft:'left',KeyA:'left',ArrowRight:'right',KeyD:'right',ArrowUp:'up',KeyW:'up',ArrowDown:'down',KeyS:'down'};if(m[e.code]){keys[m[e.code]]=on;e.preventDefault()}if(on&&!e.repeat){if(failurePopupT>0)return;if(e.code==='Digit1')tryUpgrade('hp');if(e.code==='Digit2')tryUpgrade('atk');if(e.code==='Digit3')tryUpgrade('def');if(e.code==='Digit4')tryUpgrade('spd');if(e.code==='KeyJ'){if(player.hp<=0){if(gauntletMode)startGauntlet();else reset()}else act('punch')}if(e.code==='KeyK')act('kick');if(e.code==='KeyL'){if(wave===2){if(gauntletMode)startGauntlet();else reset()}else act('grab')}if(e.code==='Space')act('jump');if(e.code==='KeyR'){if(gauntletMode)startGauntlet();else reset()}}}
  ...
  document.querySelectorAll('[data-action]').forEach(b=>b.addEventListener('pointerdown',e=>{e.preventDefault();if(failurePopupT>0)return;const a=b.dataset.action;if(wave===2&&a==='grab'){if(gauntletMode)startGauntlet();else reset()}else if(player.hp<=0&&a==='punch'){if(gauntletMode)startGauntlet();else reset()}else act(a)}));
  function beginFromMenu(starter,allowFreeTour=true){if(!prepareRecruitment())return;freeTourMode=allowFreeTour&&!!document.querySelector('#free-tour')?.checked;document.querySelector('#start-card').style.display='none';document.querySelector('#touch-ui').classList.add('active');running=true;starter()}
  document.querySelector('#start').onclick=()=>beginFromMenu(reset,true);
  document.querySelector('#level-select').onclick=()=>{const p=document.querySelector('#level-panel'),gp=document.querySelector('#gauntlet-panel'),rp=document.querySelector('#recruit-panel');gp.hidden=true;rp.hidden=true;p.hidden=!p.hidden};
  document.querySelectorAll('[data-map]').forEach(b=>b.addEventListener('click',()=>beginFromMenu(()=>{player.kills=0;mapCycle=0;loadMap(Number(b.dataset.map)||0)},true)));
  ...
  document.querySelector('#gauntlet-start').onclick=()=>{
    ...
    beginFromMenu(startGauntlet,false)};
  function refreshRecruitPanel(override=''){
    const free=!!document.querySelector('#test-free-recruit')?.checked,total=[...selectedRecruitTypes].reduce((sum,type)=>sum+(recruitCatalog.find(r=>r.type===type)?.price||0),0);
    const gold=document.querySelector('#recruit-gold'),status=document.querySelector('#recruit-status');if(gold)gold.textContent=`金币 ${progress.gold}`;
    document.querySelectorAll('.recruit-enemy').forEach(b=>{const selected=selectedRecruitTypes.has(b.dataset.type),price=Number(b.dataset.price)||0;b.classList.toggle('selected',selected);b.classList.toggle('unaffordable',!free&&!selected&&progress.gold<total+price)});
    if(status)status.textContent=override||(selectedRecruitTypes.size===0?'尚未选择队友':free?`已选择 ${selectedRecruitTypes.size} 名队友 · 测试版免费出战`:`已选择 ${selectedRecruitTypes.size} 名队友 · 共需金币 ${total}`)
  }
  (function buildRecruitPanel(){
    const box=document.querySelector('#recruit-enemies');
    recruitCatalog.forEach(({type,price})=>{const b=document.createElement('button');b.className='recruit-enemy';b.dataset.type=type;b.dataset.price=price;b.textContent=enemyCatalog[type].name;const cost=document.createElement('span');cost.textContent=`${price} 金币`;b.appendChild(cost);b.onclick=()=>{selectedRecruitTypes.has(type)?selectedRecruitTypes.delete(type):selectedRecruitTypes.add(type);refreshRecruitPanel()};box.appendChild(b)});
    document.querySelector('#test-free-recruit').addEventListener('change',()=>refreshRecruitPanel());refreshRecruitPanel()
  })();
  ...
  document.querySelector('#reward-confirm').addEventListener('click',()=>{if(pendingReward)claimRewardedResource(pendingReward.kind,pendingReward.mode)});
  document.querySelector('#reward-cancel').addEventListener('click',()=>closeRewardModal());
  document.querySelector('#growth-toggle').addEventListener('click',()=>{if(failurePopupT>0)return;const panel=document.querySelector('#growth-panel');panel.hidden=!panel.hidden});
  document.querySelector('#growth-close').addEventListener('click',()=>{document.querySelector('#growth-panel').hidden=true});
