(() => {
  ...
  function startCompanionSpecial(c,t,d){
    ...
    if(c.type==='suit'&&d<168){const backflip=c.suitSkillNext==='backflip';c.suitSkillNext=backflip?'dagger':'backflip';c.state=backflip?'suitBackflip':'suitDagger';c.timer=backflip?.92:.72;c.suitHits=backflip?null:[false,false,false,false];c.specialHit=false;return true}
  ...
  function startEnemySpecial(e,d,target=player){
    ...
    if(e.type==='suit'&&d<168){const backflip=e.suitSkillNext==='backflip';e.suitSkillNext=backflip?'dagger':'backflip';e.state=backflip?'suitBackflip':'suitDagger';e.timer=backflip?.92:.72;e.suitHits=backflip?null:[false,false,false,false];e.specialHit=false;message=backflip?'西装男压低身体，准备后手翻！':'西装男拔出匕首，连续划刺！';messageT=.65;return true}
