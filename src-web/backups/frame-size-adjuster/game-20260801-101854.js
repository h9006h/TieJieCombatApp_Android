(() => {
  ...
  function startGauntlet(){
    ...
    pickups=[{type:'hammer',x:520,y:570,level:0,active:true,throwsLeft:2},{type:'grenade',x:650,y:570,level:0,active:true}];projectiles=[];hitFx=[];blastFx=[];screenTint=0;spawnGauntletEnemy();updateCamera()
  }
  ...
  function grantEnemyReward(e){
    ...
    progress.gold+=gold;progress.chicken+=chicken;if(fruitDrop)progress.fruit++;saveProgress();
    message=e.type==='breaker'&&!blackoutActive()?`灯回来了……先杀对人了，金币 ${gold}，鸡腿 ${chicken}${fruitDrop?'，还有果实！':''}`:`捡到点东西：金币 ${gold}，鸡腿 ${chicken}${fruitDrop?'，还有果实！':''}`;messageT=e.type==='breaker'?1.45:1.15;
  }
  ...
  function update(dt){if(player.state==='climbOut'&&!player.climb){player.state='idle';player.z=0;player.vz=0;player.level=0;player.subPit=null;player.pitSafeT=1.1}if(player.climb){if((player.climb.kind==='pitOut'||player.climb.kind==='collapseUp')&&player.climb.t>=player.climb.dur+.08){player.climb.t=player.climb.dur}updateClimb(dt);updateCamera();return}if(slow>0){slow-=dt;return}player.inv=Math.max(0,player.inv-dt);player.risingInvT=Math.max(0,(player.risingInvT||0)-dt);if(player.risingInvT<=0||player.state!=='risingPunch')player.risingAirInv=false;player.pitSafeT=Math.max(0,(player.pitSafeT||0)-dt);player.comboT=Math.max(0,player.comboT-dt);player.kickComboT=Math.max(0,player.kickComboT-dt);player.risingCooldown=Math.max(0,player.risingCooldown-dt);messageT=Math.max(0,messageT-dt);flash=Math.max(0,flash-dt);screenTint=Math.max(0,screenTint-dt);if(powerSwitch)powerSwitch.flash=Math.max(0,powerSwitch.flash-dt);shake*=Math.pow(.04,dt);for(const f of hitFx)f.t-=dt;hitFx=hitFx.filter(f=>f.t>0);for(const f of blastFx)f.t-=dt;blastFx=blastFx.filter(f=>f.t>0);updateProjectiles(dt);
    ...
    for(const e of enemies){updateEnemy(e,dt);if(e.level===0){const ground=groundWalkBounds();e.y=clamp(e.y,ground.min,ground.max);pushOutOfGroundWalls(e,24)}if(!e.dead&&!e.grabbed)pushOutOfBlockers(e,24);if(!freeTourMode&&!gauntletMode&&gatePhase==='combat')e.x=clamp(e.x,stageLockX+18,stageRightX-18);grantEnemyReward(e)}
  ...
  function render(){updateCamera();g.save();g.translate(-cameraX+(Math.random()-.5)*shake,-cameraY+(Math.random()-.5)*shake);drawScene();
    ...
    const visibleEnemies=enemies.filter(e=>(!e.dead||e.timer>0)&&(player.level<0?e.level<0:e.level>=0));
    const actorDepth=a=>a.level*1000+a.y+(a.grabbed&&player.grab===a?10:0),actors=[...visibleEnemies,player].sort((a,b)=>actorDepth(a)-actorDepth(b));for(const a of actors)drawFighter(a,a===player);
  ...
  // Keep every action for these characters on one shared per-character frame
  // scale. Special strips must not introduce a second, state-specific size.
  const defaultEnemyFrameScales={skinny:1,spinner:.86,suit:1,barbarian:1.18};
  const enemyFrameScales={...defaultEnemyFrameScales};
  function loadEnemyFrameScales(raw){
    try{
      const saved=JSON.parse((raw===undefined?localStorage.getItem('tiejie-enemy-frame-scales'):raw)||'{}');
      for(const type of Object.keys(defaultEnemyFrameScales)){
        const value=Number(saved[type]);
        enemyFrameScales[type]=Number.isFinite(value)?clamp(value,.1,1.6):defaultEnemyFrameScales[type]
      }
    }catch(e){Object.assign(enemyFrameScales,defaultEnemyFrameScales)}
  }
  loadEnemyFrameScales();
  window.addEventListener('storage',event=>{if(event.key==='tiejie-enemy-frame-scales')loadEnemyFrameScales(event.newValue)});
  ...
  function drawSpecialEnemyStrip(a,sheet,shadowSheet,count,frame,x,y,face,{air=false,rage=false}={}){
    const typeScale=enemyFrameScales[a.type]??1;
  ...
  function drawGeneratedEnemySprite(a,sheet,x,y,face,state,phase){
    if(a.type==='spinner'&&state==='spinAir'&&spinnerSpinSheet.complete&&spinnerSpinSheet.naturalWidth){
      ...
      drawSpecialEnemyStrip(a,spinnerSpinSheet,fighterShadows.spinnerSpinSheet,4,frame,x,y,face,{air:true});
    ...
    if(a.type==='barbarian'&&state==='barbarianRevive'&&barbarianReviveSheet.complete&&barbarianReviveSheet.naturalWidth){
      ...
      drawSpecialEnemyStrip(a,barbarianReviveSheet,fighterShadows.barbarianReviveSheet,4,frame,x,y,face,{rage:true});
    ...
    if(a.type==='barbarian'&&state==='barbarianCharge'&&barbarianReviveSheet.complete&&barbarianReviveSheet.naturalWidth){
      ...
      drawSpecialEnemyStrip(a,barbarianReviveSheet,fighterShadows.barbarianReviveSheet,4,3,x,y,face,{rage:a.rage});
      drawEnemyBar(a,x,y-225*(a.elite?1.08:1));return
    }
  ...
  function drawSkinnyEnemySprite(a,x,y,face,state,phase){
    const scale=enemyFrameScales.skinny;
    if(state==='run'&&skinnyEnemyWalkSheet.complete&&skinnyEnemyWalkSheet.naturalWidth){drawEnemyWalkSprite(a,x,y,face,phase,skinnyEnemyWalkSheet,scale,[0,0,0,0],40,enemyVisualHeights.skinny);return}
    const count=5,sw=skinnyEnemySheet.naturalWidth/count,sh=skinnyEnemySheet.naturalHeight,dw=sw*scale,dh=sh*scale;
    let frame=0,rot=0,tx=0,ty=0,sheet=skinnyEnemySheet,sheetW=sw,sheetH=sh,drawW=dw,drawH=dh;
    ...
    else if(state==='slide'&&skinnySlideSheet.complete){sheet=skinnySlideSheet;sheetW=skinnySlideSheet.naturalWidth;sheetH=skinnySlideSheet.naturalHeight;drawW=sheetW;drawH=sheetH;frame=0;ty=3}
  ...
  function drawEnemyBar(a,x,labelY){g.save();bar(x-38,labelY-8,76,6,a.hp/a.maxHp,a.elite?'#bf7b2f':'#8f2c2b');g.fillStyle='#c9c3b7';g.font='12px sans-serif';g.textAlign='center';g.fillText(a.name,x,labelY-14);g.restore()}
  ...
  function drawPickup(p){g.save();g.translate(p.x,p.y);drawGroundShadow(p.type==='grenade'?22:34,7,5,'#03050688');g.strokeStyle='#e2b36aaa';g.lineWidth=3;g.beginPath();g.ellipse(0,-8,p.type==='grenade'?24:34,18,0,0,6.3);g.stroke();drawItemShape(p.type,0,-7,p.type==='hammer'?-.28:0,p.type==='hammer'?.9:.82);g.restore()}
  ...
  function beginFromMenu(starter,allowFreeTour=true){freeTourMode=allowFreeTour&&!!document.querySelector('#free-tour')?.checked;document.querySelector('#start-card').style.display='none';document.querySelector('#touch-ui').classList.add('active');running=true;starter()}
  ...
  document.querySelector('#gauntlet-btn').onclick=()=>{
    const gp=document.querySelector('#gauntlet-panel'),lp=document.querySelector('#level-panel');
    lp.hidden=true;gp.hidden=!gp.hidden};
