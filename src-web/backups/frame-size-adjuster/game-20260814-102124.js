(() => {
  ...
  const skillCatalog=[
    ...
    {id:'risingPunch',branch:'拳击分支',icon:'升',name:'升龙拳',cost:1,requires:'basicPunch',desc:'保留现有地面升龙拳动作与基础玩法。',release:'在地面进入跳跃下蹲蓄力时按“拳”释放。',story:''},
    ...
    {id:'blueFlame',branch:'拳击分支',icon:'拳焰',name:'拳焰',cost:3,requires:'basicPunch',desc:'无伤状态下每次命中积累攻击速度；拳焰透明度随攻速提升，满攻速时完全不透明。',release:'点亮后被动生效。保持无伤并连续命中敌人。',story:''},
    {id:'legArts',branch:'腿法分支',icon:'腿',name:'腿法',cost:1,requires:null,desc:'开放主角的普通腿法和战斗界面的“腿”按键。',release:'按“腿”发动普通腿法。',story:''},
    {id:'legFlame',branch:'腿法分支',icon:'腿焰',name:'腿焰',cost:3,requires:'legArts',desc:'为所有腿技附加腿部火焰，并将腿技的攻击距离提升为原来的两倍。',release:'点亮后被动生效；普通踢腿、空中踢和弹射飞踢均获得腿焰与双倍攻击距离。',story:''},
    {id:'launchKick',branch:'腿法分支',icon:'飞',name:'弹射飞踢',cost:3,requires:'legFlame',desc:'在跳跃起势阶段高速向前弹射飞踢；有效命中敌人后立即停止剩余横向弹射。',release:'跳跃下蹲阶段按“腿”；同时按前方向时距离稍长。',story:''},
    {id:'launchKickChain',branch:'腿法分支',icon:'连',name:'弹射飞连腿',cost:4,requires:'launchKick',desc:'弹射飞踢途中再按一次腿，可在落势前接出空中旋风鞭腿；继续按腿可连续追打，最多追加六段。',release:'弹射飞踢途中连续按“腿”续接，每段伤害与弹射飞踢相同。',story:''},
  ...
  function legAttackRange(range){return hasSkill('legFlame')?range*2:range}
  function resetLaunchKickChain(){player.launchKickChainCount=0;player.launchKickChainQueued=0}
  function queueLaunchKickChain(){
    ...
    const queued=player.launchKickChainQueued||0,used=(player.launchKickChainCount||0)+queued
    if(used>=6){message='飞连腿已到上限';messageT=.5;return true}
    if(queued>=1){message='下一段已经接上了';messageT=.4;return true}
    player.launchKickChainQueued=1
    message=`飞连腿准备第 ${used+1} 段`
    messageT=.5
    return true
  }
  function startLaunchKickChainSegment(){
    ...
    player.launchKickChainCount=(player.launchKickChainCount||0)+1
    player.state='airBackKick'
    const chainSpeed=playerAttackSpeedMul()
    player.launchKickChainDuration=1.2*chainSpeed
    player.timer=player.launchKickChainDuration
    player.z=Math.max(player.z||0,28)
    player.vz=Math.max(player.vz||0,700)
    player.knockVx=0
    combatAttack(legAttackRange(154),17,660,200*chainSpeed,0,{knockdown:true,launchVz:470,airJuggleVz:340,zReach:165,cycloneKick:'high'})
    combatAttack(legAttackRange(164),18,740,800*chainSpeed,0,{knockdown:true,launchVz:460,airJuggleVz:330,zReach:165,cycloneKick:'low'})
    message=`高低连环鞭腿 ${player.launchKickChainCount}`
    messageT=1.05
  }
  function registerPlayerHit(){if(hasSkill('starAbsorb')&&(player.starAbsorbHits||0)<20){player.starAbsorbHits=Math.min(20,(player.starAbsorbHits||0)+1);if(player.starAbsorbHits===20){message='聚势之握已充满——按“抓”吸附敌人';messageT=1.5}}if(!hasSkill('blueFlame'))return;player.cleanHits=(player.cleanHits||0)+1;if(player.cleanHits%10===0&&player.attackSpeedTier<10){player.attackSpeedTier++;message=`无伤连击 ${player.cleanHits} 次 · 攻速 ×${playerAttackSpeedMul().toFixed(1)}`;messageT=1.1}}
  ...
  function act(name,fromQueue=false){if(!running||player.hp<=0||player.climb)return;if(player.state==='starAbsorbPull')return;if(player.state==='down'&&player.z===0&&name==='punch'&&hasSkill('downRisingPunch')){startRisingPunch('down');return}if(player.state==='hurt'||player.state==='down'||player.state==='fall')return;
    ...
    if(name==='grab'){
      ...
      if(!hasSkill('grapple')){message='还没有掌握擒拿';messageT=.8;return}
    ...
    if(name==='kick'){
      ...
      if(player.kickCombo===1){player.state='kick1';player.timer=.52;combatAttack(legAttackRange(96),15,210,250,15);message='先压住他的步子';messageT=.48;return}
      if(player.kickCombo===2){player.state='kick2';player.timer=.64;combatAttack(legAttackRange(114),21,310,345,22);message='别让他站稳';messageT=.56;return}
      player.state='backKick';player.timer=.78;combatAttack(legAttackRange(148),35,730,540,31,{knockdown:true,launchVz:460});message='转过去，把力踹透！';messageT=.86;return
    }
  }
  function tryPushBunkerDoor(){
    ...
    player.state='grabAttempt';player.timer=.42;player.inputQueue.length=0;player.face=1;message=after?'石门被推开，下一间房亮起来了':'最后一道石门打开，出口露出来了';messageT=1.25;hitStop(.05);return true
  }
  function startLaunchKick(){
    ...
    combatAttack(legAttackRange(136),15,640,200*playerAttackSpeedMul(),0,{knockdown:true,launchVz:420,zReach:128,stopLaunchKickOnHit:true});message='蹬地弹射——飞踢！';messageT=.95
  }
  ...
  function update(dt){if(player.state==='climbOut'&&!player.climb){player.state='idle';player.z=0;player.vz=0;player.level=0;player.subPit=null;player.pitSafeT=1.1}if(player.climb){if((player.climb.kind==='pitOut'||player.climb.kind==='collapseUp')&&player.climb.t>=player.climb.dur+.08){player.climb.t=player.climb.dur}updateClimb(dt);updateCamera();return}if(slow>0){slow-=dt;return}player.inv=Math.max(0,player.inv-dt);player.risingInvT=Math.max(0,(player.risingInvT||0)-dt);if(player.risingInvT<=0||player.state!=='risingPunch')player.risingAirInv=false;player.pitSafeT=Math.max(0,(player.pitSafeT||0)-dt);player.comboT=Math.max(0,player.comboT-dt);player.kickComboT=Math.max(0,player.kickComboT-dt);player.risingCooldown=Math.max(0,player.risingCooldown-dt);messageT=Math.max(0,messageT-dt);flash=Math.max(0,flash-dt);screenTint=Math.max(0,screenTint-dt);if(powerSwitch)powerSwitch.flash=Math.max(0,powerSwitch.flash-dt);shake*=Math.pow(.04,dt);for(const f of hitFx)f.t-=dt;hitFx=hitFx.filter(f=>f.t>0);for(const f of blastFx)f.t-=dt;blastFx=blastFx.filter(f=>f.t>0);for(const f of resourceFx)f.t-=dt;resourceFx=resourceFx.filter(f=>f.t>0);for(const f of eliteArmorFx)f.t-=dt;eliteArmorFx=eliteArmorFx.filter(f=>f.t>0);for(const f of starAbsorbFx)f.t-=dt;starAbsorbFx=starAbsorbFx.filter(f=>f.t>0);updateProjectiles(dt);
    ...
    if(player.state==='starAbsorbPull')updateStarAbsorbPull(dt);else if(player.timer>0){const timerRate=player.launchKick&&player.state==='airBackKick'?1:playerAttackStates.has(player.state)?playerAttackSpeedMul():1;player.timer-=dt*timerRate;if(player.timer<=0){if(player.state==='jumpCrouch'){if(player.risingQueued)startRisingPunch();else{player.vz=520;player.state='jump';player.timer=.55;const queued=player.inputQueue.shift();if(queued==='punch'||queued==='kick')act(queued,true)}}else if(player.state==='jumpLand'){const queued=player.inputQueue.shift();player.state='idle';if(queued)act(queued,true)}else if(player.state==='grabAttempt'){finishGrabAttempt()}else if(player.grab&&player.state==='grabKnee'){if(player.grabKickQueue>0){player.grabKickQueue--;startKneeStrike()}else if(player.grabKickTotal>=3){releaseGrab(false,true);message='三次膝撞够狠了……他拼命挣脱了！';messageT=1}else player.state='grab'}else if(player.state==='down'&&player.z>0){player.timer=.1}else if(player.state==='down'){player.state='getUp';player.timer=.55}else if(player.state==='getUp'){const queued=player.inputQueue.shift();player.state='idle';if(queued)act(queued,true)}else if(!player.grab){if(player.state==='airBackKick'&&player.launchKickChainQueued>0&&player.z>0)startLaunchKickChainSegment();else{const queued=player.inputQueue.shift();if(player.state!=='airBackKick'||player.z<=0)resetLaunchKickChain();player.state=player.z>0?'jump':'idle';if(queued)act(queued,true)}}}}
    let dx=(keys.right?1:0)-(keys.left?1:0),dy=(keys.down?1:0)-(keys.up?1:0);if(player.z>0||player.vz!==0){player.airPrevX=player.x;dy=0}
    const moveLocked=['hurt','down','getUp','pickup','punch1','punch2','punch3','kick1','kick2','backKick','risingPunch','throwItem','throw','overChestThrow','grabAttempt','grabRelease','starAbsorbPull','enemyGrabbed','climb','jumpCrouch','jumpLand'].includes(player.state)||(player.launchKick&&player.state==='airBackKick');
    if(player.grab){const e=player.grab;player.grabT+=dt;player.grabEscapeT=Math.max(0,GRAB_MAX_DURATION-player.grabT);if(player.state==='drag')player.state='grab';if(player.state==='overChestThrow'){
      ...
      const sp=playerSpeedMul()*terrainSpeedMul(player)*floodWalkSpeedMul(player),airControl=player.z>0||player.vz!==0?1.1:1,len=Math.hypot(dx,dy)||1;dx/=len;dy/=len;if(dx)player.face=Math.sign(dx);player.x+=dx*320*sp*airControl*dt;player.y+=dy*206*sp*(airControl>1?1.25:1)*dt;if(player.z===0&&player.vz<=0&&player.state!=='jumpCrouch')player.state=(dx||dy)?'run':'idle';
    }
  ...
  function enemySpriteFilter(a,base='none'){
    ...
    // cause thermal throttling in long sessions. Keep elite styling and the
    // thin super-armor outline, but skip routine grading on normal enemies.
    const normal=base&&base!=='none'&&(!nativeDouyinRuntime||a?.elite)?base:'',gold=enemyHasSuperArmor(a)?'drop-shadow(.8px 0 0 #f6c945) drop-shadow(-.8px 0 0 #f6c945) drop-shadow(0 .8px 0 #f6c945) drop-shadow(0 -.8px 0 #f6c945) drop-shadow(0 0 1px #ffd86188)':'';
  ...
  function render(){updateCamera();g.globalAlpha=1;g.globalCompositeOperation='source-over';g.filter='none';g.shadowColor='transparent';g.shadowBlur=0;g.save();g.translate(-cameraX+(Math.random()-.5)*shake,-cameraY+(Math.random()-.5)*shake);g.save();drawScene();g.restore();
    ...
    effects();drawEliteArmorEffects();g.restore();g.beginPath();hud()}
  ...
  function drawHeroSprite(a,x,y,face,state,phase){
    ...
    const paintCycloneWhip=(frame)=>{if(frame!==1&&frame!==3)return;const high=frame===1,now=performance.now(),pulse=.9+Math.sin(now*.045)*.1;g.save();g.globalCompositeOperation='screen';g.lineCap='round';g.translate(0,-96);g.rotate(high?-.24:.18);for(let i=0;i<4;i++){const radius=82+i*18,start=-2.72,end=high?.36:.2,gradient=g.createLinearGradient(-radius,0,radius,0);gradient.addColorStop(0,'#2d7dff00');gradient.addColorStop(.38,'#55cfff88');gradient.addColorStop(.72,'#d8f8ffdd');gradient.addColorStop(1,'#ffffff22');g.globalAlpha=(.34-i*.055)*pulse;g.strokeStyle=gradient;g.lineWidth=10-i*1.6;g.beginPath();g.ellipse(12,i*4,radius,radius*(high?.44:.32),0,start,end);g.stroke()}for(let i=0;i<7;i++){const angle=-.95+i*.27+(now*.004%(Math.PI*.24)),radius=92+(i%3)*18;g.globalAlpha=.28;g.strokeStyle=i%2?'#a6ecff':'#4b9cff';g.lineWidth=2.2;g.beginPath();g.moveTo(Math.cos(angle)*radius,Math.sin(angle)*radius*.42);g.lineTo(Math.cos(angle)*(radius+34),Math.sin(angle)*(radius+34)*.42);g.stroke()}g.restore()};
    ...
    if(state.startsWith('punch')&&heroPunchComboSheet.complete){
    ...
    }else if((state==='grabAttempt'||state==='grabRelease'||state==='starAbsorbPull'||state==='grab'||state==='drag')&&heroGrabSheet.complete){
      ...
      const p=clamp(1-a.timer/.52,0,.9999),frame=Math.min(3,Math.floor(p*4)),frameY=[-1,0,0,0],anchors=[[230,320,-1.9,.7],[230,287,-1.7,.82],[271,231,-1.52,1.18],[230,288,-1.7,.82]];paintN(heroKick1Sheet,4,frame,.30,0,frameY[frame],1,'none');paintLegFlame(heroKick1Sheet,4,frame,anchors,0,frameY[frame],{prev:Math.max(0,frame-1),next:Math.min(3,frame+1)});
    }else if(state==='kick2'&&heroKick2Sheet.complete){
      const p=clamp(1-a.timer/.64,0,.9999),frame=Math.min(4,Math.floor(p*5)),frameY=[0,-1,-1,-1,0],anchors=[[232,320,-1.9,.7],[229,283,-1.72,.82],[279,230,-1.5,1.2],[278,283,-1.55,1.04],[226,320,-1.9,.7]];paintN(heroKick2Sheet,5,frame,.34,0,frameY[frame],1,'none');paintLegFlame(heroKick2Sheet,5,frame,anchors,0,frameY[frame],{prev:Math.max(0,frame-1),next:Math.min(4,frame+1)});
    }else if(state==='backKick'&&heroKick3Sheet.complete){
      const p=clamp(1-a.timer/.78,0,.9999),frame=Math.min(3,Math.floor(p*4)),frameY=[-2,-1,-1,-1],anchors=[[226,319,-1.9,.7],[229,288,-1.7,.82],[228,274,-1.62,.88],[281,263,-1.45,1.2]];paintN(heroKick3Sheet,4,frame,.371,0,frameY[frame],1,'none');paintLegFlame(heroKick3Sheet,4,frame,anchors,0,frameY[frame],{prev:Math.max(0,frame-1),next:Math.min(3,frame+1)});
    }else if(state==='airBackKick'&&(heroJumpKickSheet.complete||heroJumpKickChainSheet.complete)){
      const chainActive=!a.launchKick&&(a.launchKickChainCount||0)>0&&heroJumpKickChainSheet.complete,airKickSheet=chainActive?heroJumpKickChainSheet:heroJumpKickSheet,normalFrames=chainActive?[0,1,2,3]:[1,3,4],launchFrames=[0,1,3,5],elapsed=clamp(.5-a.timer,0,.4999),chainDuration=Math.max(.001,a.launchKickChainDuration||1.2*playerAttackSpeedMul()),chainElapsed=clamp((chainDuration-a.timer)/playerAttackSpeedMul(),0,1.1999),frame=chainActive?(chainElapsed<.2?0:chainElapsed<.6?1:chainElapsed<.8?2:3):a.launchKick?(elapsed<.1?0:elapsed<.2?1:elapsed<.4?3:5):normalFrames[Math.min(normalFrames.length-1,Math.floor(clamp(1-a.timer/.62,0,.9999)*normalFrames.length))],flowFrames=chainActive?normalFrames:(a.launchKick?launchFrames:normalFrames),flowIndex=Math.max(0,flowFrames.indexOf(frame)),airKickFrameY=chainActive?[0,0,0,0]:[0,-82,-62,-46,-76,0],anchors=chainActive?[[194,292,-.35,.92],[286,224,-1.34,1.38],[193,304,-2.15,.9],[292,274,-1.48,1.34]]:[[205,295,-1.85,.75],[232,284,-1.68,.86],[229,273,-1.6,.9],[282,262,-1.45,1.25],[229,286,-1.7,.86],[212,295,-1.82,.76]],frameCount=chainActive?4:6,kickOutFrame=!chainActive&&frame===3,kickOutFlameRotation=kickOutFrame?-LEG_FLAME_SOURCE_HEAD_ANGLE:null,kickOutFlameY=airKickFrameY[frame]+(kickOutFrame?10:0);if(chainActive)paintCycloneWhip(frame);paintN(airKickSheet,frameCount,frame,.41,0,airKickFrameY[frame],1,'none');paintLegFlame(airKickSheet,frameCount,frame,anchors,0,kickOutFlameY,{prev:flowFrames[Math.max(0,flowIndex-1)],next:flowFrames[Math.min(flowFrames.length-1,flowIndex+1)],rotation:kickOutFlameRotation});
  ...
  function drawHeavyEnemySprite(a,x,y,face,state,phase){
    ...
    if(useHeavyDeathFrame&&heavyEnemyDeathSheet.complete&&heavyEnemyDeathSheet.naturalWidth){
      ...
      g.save();g.translate(x,y);g.scale(face*(enemyFrameFlip('heavyDeath',0)?-1:1),1);g.translate(offsetX,offsetY);g.filter=enemySpriteFilter(a,a.elite?'sepia(.28) saturate(1.35) hue-rotate(330deg) brightness(.92) contrast(1.08)':'none');g.drawImage(heavyEnemyDeathSheet,-size*.5,-size+40*(a.elite?1.1:1),size,size);g.filter='none';g.restore();return
    }
  ...
  function drawStarAbsorbEffects(){
    for(const f of starAbsorbFx){
      ...
      g.globalAlpha=fade*(f.success?.28:.62);g.fillStyle='#6d83ff22';g.strokeStyle='#96eaff';g.lineWidth=2.5;g.setLineDash([14,9]);g.lineDashOffset=-q*80;g.beginPath();g.moveTo(f.centerX,f.centerY);for(let i=0;i<=18;i++){const a=baseAngle-halfAngle+halfAngle*2*i/18;g.lineTo(f.centerX+Math.cos(a)*fanRadius,f.centerY+Math.sin(a)*fanRadius/1.35)}g.closePath();g.fill();g.stroke();g.setLineDash([]);
      if(!f.success){
        for(let i=0;i<18;i++){const a=baseAngle-halfAngle+halfAngle*2*(i+.5)/18,r=fanRadius*(.82+(i%3)*.08),x=f.centerX+Math.cos(a)*r,y=f.centerY+Math.sin(a)*r/1.35;g.strokeStyle=i%2?'#c59aff':'#62dfff';g.lineWidth=2+(i%3);g.beginPath();g.moveTo(x,y);g.lineTo(x+(f.toX-x)*.2,y+(f.toY-y)*.2);g.stroke()}
      }else{
        const sourceContract=42*(1-q)+8;
        g.globalAlpha=fade*(.45+.55*pulse);g.strokeStyle='#cc9cff';g.lineWidth=3+3*pulse;g.beginPath();g.ellipse(f.fromX,f.fromY,sourceContract,sourceContract*.55,0,0,Math.PI*2);g.stroke();
        for(let i=0;i<10;i++){const lane=(i-4.5)*5.5,wave=Math.sin(f.seed+i*1.83+q*16)*10*(1-q*.55),sx=f.fromX+nx*lane,sy=f.fromY+ny*lane,tx=f.toX,ty=f.toY,cx=(sx+tx)*.5+nx*(wave+lane*.35),cy=(sy+ty)*.5+ny*(wave+lane*.35),stream=g.createLinearGradient(sx,sy,tx,ty);stream.addColorStop(0,i%2?'#b77aff22':'#48ccff22');stream.addColorStop(.45,i%2?'#d5aaffcc':'#61e8ffcc');stream.addColorStop(1,'#f4f0ff');g.globalAlpha=fade*(.48+.52*pulse);g.strokeStyle=stream;g.lineWidth=i%4===0?7:2.5;g.beginPath();g.moveTo(sx,sy);g.quadraticCurveTo(cx,cy,tx,ty);g.stroke()}
        for(let i=0;i<22;i++){const p=clamp(q*1.42-(i/22)*.54,0,1),arc=Math.sin(p*Math.PI)*Math.sin(f.seed+i*2.17)*20,x=f.fromX+dx*p+nx*arc,y=f.fromY+dy*p+ny*arc,tail=10+(i%4)*3;g.globalAlpha=fade*(.5+.5*pulse);g.strokeStyle=i%2?'#e1c3ff':'#a8f4ff';g.lineWidth=1.5+(i%3);g.beginPath();g.moveTo(x-dx/distance*tail,y-dy/distance*tail);g.lineTo(x,y);g.stroke()}
      }
      const vortex=18+30*pulse;g.globalAlpha=fade;g.shadowColor='#8aeaff';g.shadowBlur=18+18*pulse;g.strokeStyle='#ecddff';g.lineWidth=4;g.beginPath();g.arc(f.toX,f.toY,vortex,-q*9,-q*9+Math.PI*1.55);g.stroke();g.strokeStyle='#68dcff';g.lineWidth=2;g.beginPath();g.arc(f.toX,f.toY,vortex*.58,q*12,q*12+Math.PI*1.45);g.stroke();g.fillStyle='#f7f3ff';g.beginPath();g.arc(f.toX,f.toY,4+7*pulse,0,Math.PI*2);g.fill();g.shadowBlur=0;g.restore()
    }
  }
