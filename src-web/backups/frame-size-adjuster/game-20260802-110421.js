(() => {
  ...
  const {
    ...
    spinnerSpinSheet,grapplerWrestlingSheet,barbarianReviveSheet,barbarianQiAuraSheet,barbarianSwingFxSheet,
  ...
  let running=false,last=0,shake=0,flash=0,slow=0,wave=1,message='',messageT=0,screenTint=0,hitFx=[],blastFx=[],resourceFx=[],cameraX=0,cameraY=0,mapIndex=0,gateIndex=0,mapCycle=0,gatePhase='combat',stageLockX=70,stageRightX=1050,cameraLockX=0,powerSwitch=null,gauntletMode=false,gauntletIndex=0,freeTourMode=false,rankedRunEligible=false,failureHandled=false,failurePopupT=0,partyDefeatPending=false,lostResources=null,pendingReward=null,rewardResumeRunning=false,battleMenuResumeRunning=false,stageRewardTotals={gold:0,chicken:0,fruit:0},stageChickenBudget=1,stageChickenDropped=0,stageSettlementOpen=false,stageRewardDoubled=false,jumpSourcePlatform=null,jumpPlatformCleared=false;
  ...
  const defaultProgress={gold:0,chicken:0,fruit:0,hpLv:0,atkLv:0,defLv:0,spdLv:0,ascend:0,bestStage:0,currentStage:1};
  let progress=loadProgress();
  function loadProgress(){try{const saved=JSON.parse(localStorage.getItem('tiejie-progress')||'{}'),next={...defaultProgress,...saved};next.bestStage=Math.max(0,Math.floor(Number(next.bestStage)||0));next.currentStage=Math.max(1,Math.floor(Number(next.currentStage)||next.bestStage||1));return next}catch(e){return{...defaultProgress}}}
  ...
  function setStagePosition(stage,persist=true){const value=clamp(Math.floor(Number(stage)||1),1,maxSelectableStage()),zero=value-1;mapCycle=Math.floor(zero/trialMaps.length);mapIndex=zero%trialMaps.length;if(persist){progress.currentStage=value;saveProgress()}return value}
  ...
  function tryUpgrade(kind){
    ...
    if(progress.chicken<1){openRewardModal('chicken',`提升${m[1]}需要 1 个鸡腿，当前没有鸡腿。`);return}
    progress.chicken--;progress[m[0]]++;player.maxHp=playerMaxHp();if(kind==='hp')player.hp=player.maxHp;message=`${m[1]} +1`;messageT=.9;saveProgress()
  }
  ...
  function openStageSettlement(){
    ...
    if(clearedStage>progress.bestStage){progress.bestStage=clearedStage;saveProgress();if(rankedRunEligible)window.TieJiePlatform?.rank?.submitHighestStage?.(clearedStage)}
    ...
    summary.hidden=false;confirm.textContent='看广告，奖励翻倍';cancel.textContent=mapIndex<trialMaps.length-1?'直接领取并进入下一关':'直接领取并进入下一轮';cancel.hidden=false;if(exit)exit.hidden=true;modal.hidden=false
  }
  ...
  function revivePartyFull(){
    ...
    player.hp=player.maxHp;Object.assign(player,{state:'idle',timer:0,z:0,vz:0,knockVx:0,throwVx:0,airLaunch:false,inv:2,grab:null,grabTarget:null,grabbed:false,grappleHolder:null,grappleInvincible:false});
  ...
  function returnToStart(){
    ...
    enemies=[];companions=[];projectiles=[];hitFx=[];blastFx=[];resourceFx=[];message='';messageT=0;refreshStartStageLabel()
  }
  ...
  function hurtCompanion(c,dmg,kx,options={}){
    if(c.dead||c.grappleInvincible||(c.inv>0&&!options.ignoreInv))return;const held=!!c.grappleHolder,real=Math.max(1,Math.round(dmg*(c.defMul||1))),blade=options.sourceType==='axe'||options.sourceType==='assassin';c.hp-=real;c.inv=.32;if(!held)c.x+=kx*.35;if(blade)window.TieJieAudio?.hitPlayer(real,options.sourceType);else window.TieJieAudio?.hitEnemy(c.type,real,false);impact(c.x,c.y-c.z-76,real,false,kx);
    if(c.hp<=0&&c.type==='barbarian'&&!c.revived){c.hp=0;c.state='barbarianDown';c.timer=3;c.inv=3;c.z=0;c.vz=0;c.knockVx=0;return}
    if(c.hp<=0){c.hp=0;c.dead=true;c.state='down';c.timer=2.2;message=`${c.name}倒下了，下张地图会归队`;messageT=1.2}else if(held){c.state='grappleHeld';c.timer=Math.max(c.timer,.18)}else if(options.knockdown){c.state='down';c.timer=.9;c.z=Math.max(c.z||0,18);c.vz=Math.max(c.vz||0,options.launchVz||340)}else{c.state='hurt';c.timer=.3}
  }
  ...
  function pinGrappleVictim(holder,victim,trip=false){
    if(!holder||!victim||victim.dead||victim.hp<=0)return false;
  ...
  function startCompanionSpecial(c,t,d){
    ...
    if(c.type==='grappler'&&!t.grabbed&&d<145&&Math.abs(t.y-c.y)<64){c.state='enemyGrabWindup';c.timer=.34;return true}
  ...
  function loadMap(index){
    ...
    const maxHp=playerMaxHp();mapIndex=index;progress.currentStage=currentStageNumber();saveProgress();stageRewardTotals={gold:0,chicken:0,fruit:0};stageSettlementOpen=false;stageRewardDoubled=false;window.TieJieAudio?.playMusic(mapIndex);gateIndex=0;gatePhase='combat';stageLockX=70;stageRightX=1050;cameraLockX=0;cameraX=0;currentStageTheme=trialMaps[mapIndex].theme;generatedTerrain=generateFreshTerrain(currentStageTheme,mapIndex);Object.assign(player,{x:220,y:566,z:0,vz:0,knockVx:0,throwVx:0,airLaunch:false,level:0,face:1,hp:maxHp,maxHp,state:'idle',timer:0,combo:0,comboT:0,kickCombo:0,kickComboT:0,grab:null,grabTarget:null,grabbed:false,grappleHolder:null,grappleInvincible:false,grabT:0,grabEscapeT:0,grabKickQueue:0,grabKickTotal:0,risingQueued:false,risingCooldown:0,risingAirInv:false,risingInvT:0,held:null,heldUses:0,inv:0,kills:player.kills||0,inputQueue:[],climb:null,fallDamage:0,fallFromLevel:null,fallStartY:null,subPit:null,pitSafeT:0});
  ...
  function advanceStage(){
    const next=currentStageNumber()+1;setStagePosition(next);window.TieJiePlatform?.ads?.showInterstitial?.();loadMap(mapIndex);message=`进入第 ${next} 关 · 难度 ×${stageDifficulty(next).toFixed(1)}`;messageT=2;return;
  }
  ...
  function startEnemySpecial(e,d,target=player){
    ...
    if(e.type==='grappler'&&!target.grabbed&&d<145&&Math.abs(target.y-e.y)<64){e.state='enemyGrabWindup';e.timer=.34;message='擒拿手贴近目标了';messageT=.65;return true}
  ...
  const enemyAttackStates=new Set(['slamCharge','slamAir','slamLand','slideWindup','slide','spinAir','enemyGrabWindup','enemyGrabbed','grappleTrip','enemyThrow','axeWindup','axeSlash','teleport','stab','suitCombo','whipWindup','whipStrike','lightsOutCast','barbarianRevive','barbarianCharge','barbarianUppercut','breakerCurrent']);
  ...
  function drawHeroSprite(a,x,y,face,state,phase){
    ...
    const paintN=(sheet,count,frame,scale,tx=0,ty=0,sx=1,filter='none',rot=0)=>{scale=characterFrameScale(sheet,frame);const sw=sheet.naturalWidth/count,sh=sheet.naturalHeight,dw=sw*scale,dh=sh*scale;paintShadow(sheet,count,frame,scale,1,tx,sx);g.save();g.filter=filter;g.translate(tx,ty);g.rotate(rot);g.scale(sx,1);g.drawImage(sheet,frame*sw,0,sw,sh,-dw*.5,-dh+40*scale,dw,dh);g.restore()};
    ...
    if(state.startsWith('punch')&&heroPunchComboSheet.complete){
      ...
      const p=clamp(1-a.timer/.62,0,.9999),frames=[1,2,3,4],frame=frames[Math.min(frames.length-1,Math.floor(p*frames.length))],airKickFrameY=[0,-82,-62,-46,-76,0];paintN(heroJumpKickSheet,6,frame,.41,0,airKickFrameY[frame],1,'none');
  ...
  function drawEnemyBar(a,x,labelY){g.save();const referenceHp=enemyCatalog.skinny.hp||100,style=a.companion?{fill:'#3fa768',edge:'#9ce7b4',h:8}:enemyBarStyles[a.type]||enemyBarStyles.skinny,baseMaxHp=a.maxHp/Math.max(1,a.difficulty||1),w=clamp(68+Math.log2(Math.max(1,baseMaxHp/referenceHp))*20+(a.elite?18:0),68,230),h=style.h+(a.elite?2:0),left=x-w*.5,top=labelY-8,p=clamp(a.hp/a.maxHp,0,1);g.fillStyle='#07090bdd';g.fillRect(left-2,top-2,w+4,h+4);g.strokeStyle=style.edge;g.lineWidth=a.elite?2:1;g.strokeRect(left-1,top-1,w+2,h+2);g.fillStyle=style.fill;g.fillRect(left,top,w*p,h);g.fillStyle='#ffffff30';g.fillRect(left+1,top+1,Math.max(0,w*p-2),Math.max(1,Math.floor(h*.28)));if(a.type==='heavy'||a.type==='barbarian'){g.strokeStyle='#080a0c99';g.lineWidth=1;for(let i=1;i<5;i++){const sx=left+w*i/5;g.beginPath();g.moveTo(sx,top);g.lineTo(sx,top+h);g.stroke()}}g.fillStyle=a.companion?'#baf5ca':a.elite?'#ffd28a':'#ddd4c2';g.font=`${a.elite?'800':'700'} 12px sans-serif`;g.textAlign='center';g.fillText(a.name,x,labelY-15);g.fillStyle='#e8e1d5';g.font='10px sans-serif';g.fillText(`${shortStat(a.hp)} / ${shortStat(a.maxHp)}`,x,labelY+h+12);g.restore()}
  ...
  function hud(){g.fillStyle='#091014dd';g.fillRect(28,24,460,106);g.strokeStyle='#8a5638';g.strokeRect(28,24,460,106);g.fillStyle='#e8d5b3';g.font='800 21px sans-serif';g.textAlign='left';g.fillText('陆骁',48,54);bar(48,67,270,15,player.hp/player.maxHp,'#c84932');g.fillStyle='#96a5a5';g.font='14px sans-serif';g.fillText(`HP ${Math.max(0,player.hp)} / ${player.maxHp}`,328,80);g.fillStyle='#d5a467';g.fillText(player.held?(player.held==='grenade'?'手持：手榴弹':`手持：战锤（剩 ${player.heldUses} 次）`):'徒手格斗',48,94);
    ...
    const place=levelName(player.level),map=trialMaps[mapIndex],hudRight=W-40;g.textAlign='right';g.fillStyle='#ddd';g.font='700 17px sans-serif';if(freeTourMode)g.fillText(`自由游览 · 第 ${currentStageNumber()} 关`,hudRight,49);else if(gauntletMode)g.fillText(`测试轮战 ${Math.min(gauntletIndex+1,gauntletEnemyTypes.length)} / ${gauntletEnemyTypes.length}`,hudRight,49);else g.fillText(`第 ${currentStageNumber()} 关 · 难度 ×${stageDifficulty().toFixed(1)} · 路段 ${gateIndex+1}/${map.gates.length}`,hudRight,49);g.fillStyle='#91a0a1';g.font='14px sans-serif';g.fillText(`${gauntletMode?'模型测试场':map.name} · ${themeName(currentStageTheme)} · ${place}`,hudRight,72);g.fillText(freeTourMode?`队友 ${companions.length} · 敌人关闭`:`击倒 ${player.kills} · 队友 ${companions.filter(c=>!c.dead).length}/${companions.length} · 最高 ${progress.bestStage} 关`,hudRight,94);
  ...
  async function beginFromMenu(starter,allowFreeTour=true,rankedRun=true){
    ...
    window.TieJieAudio?.unlock();freeTourMode=allowFreeTour&&!!document.querySelector('#free-tour')?.checked;rankedRunEligible=rankedRun&&!freeTourMode&&!(selectedRecruitTypes.size>0&&document.querySelector('#test-free-recruit')?.checked);document.querySelector('#start-card').style.display='none';document.querySelector('#touch-ui').classList.add('active');document.querySelector('#battle-menu-toggle').hidden=false;document.querySelector('#battle-menu').hidden=true;starter();running=true;battleStarting=false
  }
